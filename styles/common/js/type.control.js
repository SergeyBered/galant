function typeControl(_typeId, _options) {
	...
	var init = function() {
		...
		jQuery("a", addButton).on('click', function() {
	...
	this.addGroup = function(_options) {
		if(_options.id) {
				...
				jQuery("<div class=\"fg_container\">\
					...
					<div class=\"group_edit\" style='display:none;'>\
						...
						<div class='buttons'>\
							...
							<span class='l'></span><span class='r'></span></div>\
			...
			jQuery("a.add", groupContainer).on('click', function() {
			...
			jQuery("#"+gid+"title, #"+gid+"name", groupContainer).on('keyup', function(event) { okButton.attr('disabled', event.currentTarget.value.length == 0); } );
			jQuery("#"+gid+"title", groupContainer).on('focus', function(event) {													
													if(!nameInput.val().length)
														jQuery(event.currentTarget).on('keyup', {nameField : nameInput}, universalTitleConvertCallback);
											}).on('blur', function(event) {
													jQuery(event.currentTarget).off('keyup', universalTitleConvertCallback);
											});
			jQuery("input.ok", groupContainer).on('click',
					function () {
						saveGroup(_options.id, groupContainer);
						jQuery("div.fg_container_body, div.group_edit", groupContainer).slideToggle();
					} );
			jQuery("input.cancel", groupContainer).on('click',
					function () {
						if(_options.id == 'new') {
			...
			jQuery("input:text", groupContainer).on('keydown',
					function(e) {
						if(e.keyCode == 13) {
							jQuery("input.ok", groupContainer).trigger('click');
							e.stopPropagation();
							return false;
						}
					} );
			jQuery("a.edit", groupContainer).on('click',
					function () {
						jQuery("div.fg_container_body, div.group_edit", groupContainer).slideToggle();
						return false;
					} );
			jQuery("a.remove", groupContainer).on('click',
					function () {
						...
						jQuery("input.RemoveConfirmYes").on('click', function() {
	...
	this.addField = function(groupId, _options) {
		if(groupId && _options.id) {
			...
			jQuery("#"+fid+"title, #"+fid+"name", fieldContainer).on('keyup', function(event) { okButton.attr('disabled', event.currentTarget.value.length == 0); } );
			jQuery("#"+fid+"title", fieldContainer).on('focus', function(event) {
													if(!nameInput.val().length)
														jQuery(event.currentTarget).on('keyup', {nameField : nameInput}, universalTitleConvertCallback);
											}).on('blur', function(event) {
													jQuery(event.currentTarget).off('keyup', universalTitleConvertCallback);
											});
			jQuery("input.ok", fieldContainer).on('click',
					function () {
						saveField(_options.id, fieldContainer);
						jQuery("div.edit", fieldContainer).slideUp();
					} );
			jQuery("input.cancel", fieldContainer).on('click',
					function () {
						if(_options.id == 'new') {
			...
			jQuery("input:text", fieldContainer).on('keydown',
					function(e) {
						if(e.keyCode == 13) {
							jQuery("input.ok", fieldContainer).trigger('click');
							e.stopPropagation();
							return false;
						}
					} );
			jQuery("a.edit", fieldContainer).on('click',
					function () {
						jQuery("div.edit", fieldContainer).slideToggle("normal", function() { loadTypesInfo(_options.id, fieldContainer); } );
						return false;
					} );
			jQuery("a.remove", fieldContainer).on('click',
					function () {
						jQuery.openPopupLayer({
							name   : 'removeConfirm',
							target : 'removeConfirm',
			...
			jQuery("#" + fid + "type", fieldContainer).on('change',
					function() {
			...
			if(_options.id == 'new') {
				...
				window.scrollTo(offset.left,offset.top);
				jQuery("a.edit", fieldContainer).trigger('click');
		...
		if(id == 'new') {
			...
			jQuery.post("/admin/" + (formMode ? "webforms" : "data") + "/isFieldExist/.json", {
					...
					if(data.data.isExist == 'false') {
						...
						html += "<div class='eip_buttons'><input id='js-data-add-field' type='button' class='ok' value='"+getLabel('js-data-add-field')+"'></div>";
						...
						jQuery('#js-data-add-field-cancel').on('click', function() {
							closeDialog("saveFieldConfirm");
							jQuery('li[umifieldid^="new"]').remove();
						});
						jQuery('#js-data-add-field').on('click', function() {
							saveNewField(true)
						});
					} else {
						saveNewField(false);
