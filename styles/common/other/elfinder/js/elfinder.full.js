				...
				if (data.type == 'html') {
					...
					atag.each(function(){
						...
						if (text = $(this).text()) {
							...
							if (loc.href && (atag.length === 1 || ! loc.pathname.match(/(?:\.html?|\/[^\/.]*)$/i) || text.trim().match(/\.[a-z0-9-]{1,10}$/i))) {
				...
				if (data.mode == 'makebtn') {
					...
					if (data.expires) {
						f0.data('funcexpup', function() {
							...
							if (rem < 3) {
								f0.parent().children('.elfinder-button-icon-reload').trigger('click');
							} else {
...
elFinder.prototype.resources = {
	...
	tpl : {
		...
		navdir     : '<div class="elfinder-navbar-wrapper{root}"><span id="{id}" class="ui-corner-all elfinder-navbar-dir {cssclass}"><span class="elfinder-navbar-arrow"/><span class="elfinder-navbar-icon" {style}/>{symlink}{permissions}{name}</span><div class="elfinder-navbar-subtree" style="display:none"></div></div>',
		placedir   : '<div class="elfinder-navbar-wrapper"><span id="{id}" class="ui-corner-all elfinder-navbar-dir {cssclass}" title="{title}"><span class="elfinder-navbar-arrow"/><span class="elfinder-navbar-icon" {style}/>{symlink}{permissions}{name}</span><div class="elfinder-navbar-subtree" style="display:none"></div></div>'
				...
				.on('click', function(e) { 
					if (!button.hasClass(disabled)) {
						if (menu && cmd.variants.length >= 1) {
							// close other menus
							menu.is(':hidden') && fm.getUI().trigger('click');
							e.stopPropagation();
				...
				if ($.inArray(code, [ESC, ENT, LEFT, RIGHT, UP, DOWN]) !== -1) {
					...
					if (code == ESC || code === sublev) {
						...
						if (selected.hasClass('elfinder-contextmenu-group')) {
							if (subselected) {
								code == ENT && subselected.trigger('click');
							} else {
							...
							code == ENT && selected.trigger('click');
						}
					}
				}
			},
			...
			while (cnt--) {
				...
				if (fm.UA.Mobile || !inline) {
					if (html5dl) {
						...
						link.attr('href', url).get(0).trigger('click');
					} else {
						wnd = window.open(url);
						if (!wnd) {
	...
	this.window = $('<div class="ui-front ui-helper-reset ui-widget elfinder-quicklook touch-punch" style="position:absolute"/>')
		...
		.on('close', function(e, dfd) {
			...
			if (self.opened()) {
				...
				if (! self.docked()) {
					state = animated;
					win.hasClass(fullscreen) && fsicon.trigger('click');
...
(elFinder.prototype.commands.reload = function() {
	...
	this.exec = function() {
		...
		if (!search) {
			...
			$('div.elfinder-toolbar > div.'+fm.res('class', 'searchbtn') + ' > span.ui-icon-search').trigger('click');
		}
	};
