$(function() {
	...
	//Инфоблок
	$('.infoblock-hide').on('click', function() {
		hideInfoBlock(this, '.infoblock-show');
	});

	$('.infoblock-show').on('click', function() {
	...
	$('.group-tip-hide').on('click', function() {
		hideInfoBlock(this, '.group-tip-show');
	});

	$('.group-tip-show').on('click', function() {
		var button = $(this);
	...
	if ($("a.extended_fields_expander").length > 0) {
		markExtendedFields(function() {
			...
			extendedFieldToggler.on('click', function() {
	...
	if (window.is_page) {
		...
		nameInput.on('focus', function() {
			if (window.is_new || (h1Input.val() === nameInput.val()) || altNameInput.val() == '') {
				jQuery(this).on("keyup", callback);
			}

		}).on('blur', function() {
			jQuery(this).off("keyup", callback);
		});
	...
	$(function() {
		...
		DataSet.addEventHandler('onInitComplete', function() {
			...
			$('#is-active-control').on('click', function() {
	...
	$('#share-toggler').on('click', function() {
...
function initSelectizer() {
	$('.default.newselect').each(function() {
		...
		if ($this.parent('.domains_selector').length === 0) {
	...
	return function(arrData) {
		...
		function moveEntity() {
			...
			if (Method == 'tree_copy_element') {
				Params['copy_all'] = jQuery('#copy-all').is(':checked') ? 1 : 0;
			}
...
function initFileManager() {
	...
	$("div#filemanager_upload_files a").on('click', function() {
...
function initOptionedFields() {
	...
	function bindRemoveHandler(container) {
		$('a.remove-option', container).on('click', function() {
	...
	function openDialog(text, title, options) {
		...
		var popupParams = {
			...
			success: function() {
				...
				$('#confirm-button', popupId).on('click', function() {
					options.confirmCallback(options.name, popupId);
				});
				$('#cancel-button', popupId).on('click', function() {
					options.cancelCallback(options.name, popupId);
					closeDialog(options.name);
				});
				$('#close-button', popupId).on('click', function() {
					closeDialog(options.name);
				});
...
function initCreateTypeSelector(domainId) {
	jQuery("div.imgButtonWrapper a:not(.type_select)").filter(function() {
		...
		if (e.attr("umi:prevent-default") == "true") {
			...
			e.on({click: f, mousedown: f, mouseup: f, mouseover: f});
		}
...
var sliderForNavigation = function() {
	...
	$('.slider_container .right_button').on('mouseenter mouseleave', function() {
		sliderTimer = setInterval(nextSlide, 2);
	}, function() {
		clearInterval(sliderTimer);
	});

	$('.slider_container .left_button').on('mouseenter mouseleave', function() {
...
$(function() {
	var widthBlocks = $('.container_tabs .tabs.editing').width();
	var widthContainer = $('.container_tabs').width();
	$('.icon-action.extended_fields_expander').on('click', function() {
		if (widthBlocks > widthContainer) {
			$('.slider_container .left_button').addClass('active');
			$('.slider_container .right_button').addClass('active');
		} else {
