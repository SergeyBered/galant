	...
	jQuery(function(){
		jQuery("#mark_read").on('click', markReadEvents);
		jQuery("#mark_unread").on('click', markUnreadEvents);
	});
	...
	function saveSettings() {
		...
		jQuery.post(
			...
			.done(function() { reload('', startDate, endDate, '', ''); })
		...
		if (pages > 1) {
			...
			for (var i = 0; i < pages; i++) {
				...
				jQuery('<a href="?p=' + i +'&start_date=' + startDate + '&end_date=' + endDate + '&filter=' + filter + '" class="' + aclass + '">' + pagenum + '</a>')
					.on('click', onClick(i, startDate, endDate, filter))
					.appendTo(pages_bar);
			}
		}
	}
	...
	function filterType(typeId) {
		...
		child.animate({
			...
			jQuery('#' + typeId +'-child .tableDashboard').append('<div class="cont_pages_bar"><a href="javascript:void(0);" onclick="javascript:showAll(\'' +  typeId + '\', \'' + initialHeight + '\', \'' + initialWidth + '\');" class="show_all"></div><div class="pages-bar"></div></div>');
	...
	function changeReadEvents(obj) {
		...
		if (jQuery(obj).prop('checked')) {
			...
			if (jQuery(obj).prop('class') === 'unread') {
			...
			if (jQuery(obj).prop('class') === 'unread' && jQuery('.unread:checked').length === 0) {
				readButton.attr('disabled', 'disabled');
				readButton.parent().removeClass('act');
			}
			if (jQuery(obj).prop('class') === 'read' && jQuery('.read:checked').length === 0) {
		...
		jQuery.post(
			...
			.done(function() {
				...
				if (id) {
					...
					inputs.attr('class', 'read').prop("checked", false);
					clearInputs('read');
				}
	...
	function markUnreadEvents() {
		...
		jQuery.post(
			...
			.done(function() {
				...
				inputs.attr('class', 'unread').prop("checked", false)
				jQuery(".preloader").remove();
				clearInputs('unread');
			})
			.fail(function(jqXHR, textStatus, errorThrown) { alert( textStatus + ': '+ errorThrown); })
