	...
	if (typeof cm_img !== 'function') {
		cm_img = function (img, alt, style) {/* {{{ */
			...
			if (img.indexOf('.')>-1) {
				...
				return '<i class="' + (img !== 'undefined' ? 'small-ico ' + img : 'small-ico-empty' ) + '"  alt="' + alt + '"></i>';
			}
		};
	}
	...
	**/
	...
	jQuery.cmenu = {
		...
		show: function (menu, parent, oEvent) {			/* Show menu m near parent object p	{{{ */
			...
			if (!this.hideBinded) {
				...
				jQuery(document).on('click', this.hideAll);
			}
		...
		hideAll: function () {			/* Hide all displayed menus	{{{ */
			...
			jQuery(document).off('click', jQuery.cmenu.hideAll);
	...
	jQuery.fn.bindMenu = function (event, menu) {/* jQuery-plugin for menu binding	{{{ */
		...
		return this.each(function () {
			jQuery(this).on(event, function (oEvent) {
