...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- Шаблон для вкладки "Авторы" -->
	<xsl:template match="result[@method = 'authorsList']/data">
		<div class="location">
			<div class="imgButtonWrapper loc-left">
				<a href="{$lang-prefix}/admin/&sys-module;/add/author/" class="btn color-blue">
					<xsl:text>&label-add-author;</xsl:text>
				</a>
			</div>
			<xsl:call-template name="entities.help.button" />
		</div>
		<div class="layout">
			<div class="column">
				<xsl:call-template name="ui-smc-table">
					<xsl:with-param name="control-params">author</xsl:with-param>
					<xsl:with-param name="content-type">objects</xsl:with-param>
				</xsl:call-template>
			</div>
			<div class="column">
