			...
			$(function (){
				$('#generateView').on('click', function(){
					...
					$.getJSON('/admin/search/isExistsConfig/', function (data) {
						...
						var genConfigText = '';
						if (isConfig) {
							genConfigText = '<div class="checkbox sphinx"><input id="genConfigSphinx" type="checkbox" name="genConfigSphinx" class="checkbox"></div></div> <span>' +
						...
						openDialog('', getLabel('js-search-generate-config'), {
							...
							openCallback: function() {
								jQuery('.checkbox.sphinx').on('click', function() {
									jQuery(this).toggleClass('checked');
								})

								if (!isConfig) {
									jQuery('#confirm-button').trigger('click');
								}
							},
