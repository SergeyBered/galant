...
<xsl:stylesheet
	...
	<xsl:template match="data[@type = 'form' and (@action = 'modify' or @action = 'create')]">
		...
		<div class="tabs-content module {$module}-module">
			<div class="section selected">
				...
				<xsl:apply-templates select="." mode="form.wrapper">
					<xsl:with-param name="type-id">
						<xsl:value-of select="$value" />
					</xsl:with-param>
				</xsl:apply-templates>
	...
	<!-- Шаблон обертки формы редактирования или создания страницы или объекта -->
	<xsl:template match="data[@type = 'form' and (@action = 'modify' or @action = 'create')]" mode="form.wrapper">
		<xsl:param name="type-id"/>
		<xsl:apply-templates select="." mode="form">
			<xsl:with-param name="type-id">
				<xsl:value-of select="$type-id" />
			</xsl:with-param>
			<xsl:with-param name="show-revisions">
				<xsl:value-of select="string(number(@action = 'modify' and count(page) = 1))" />
			</xsl:with-param>
			<xsl:with-param name="show-create-button">
				<xsl:value-of select="string(number($data-action = 'create'))" />
			</xsl:with-param>
			<xsl:with-param name="form-action">
				<xsl:text>do/</xsl:text>
			</xsl:with-param>
		</xsl:apply-templates>
	</xsl:template>

	<!-- Шаблон формы редактирования или создания страницы или объекта -->
	<xsl:template match="data[@type = 'form' and (@action = 'modify' or @action = 'create')]" mode="form">
		<xsl:param name="type-id"/>
		<xsl:param name="show-revisions"/>
		<xsl:param name="show-create-button"/>
		<xsl:param name="form-action"/>
		<form class="form_modify" data-type-id="{$type-id}" method="post" action="{$form-action}" enctype="multipart/form-data">
	...
	<xsl:template match="data[@type = 'form' and (@action = 'modify' or @action = 'create')]" mode="form-modify-toolbar-buttons">
		...
		<xsl:if test="$isPage = 'true'">
			...
			<div class="pull-right">
				<span id="ya_share1" style="display:none;"/>
				<a class="icon-action pull-right" id="share-toggler" title="&label-share;">
					<i class="small-ico i-share"/>
				</a>
			</div>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="group[position() = 1 and count(../../basetype) = 0]" mode="form-modify-group-fields">
		...
		<xsl:if test="$show-name = '1'">
			<xsl:apply-templates select="." mode="form-modify-group-fields-name"/>
		</xsl:if>
	...
	<xsl:template
		...
		<xsl:param name="required">
			<xsl:value-of select="number(./@required = 'required')" />
		</xsl:param>
		...
		<div class="{$class} default-empty-validation">
			<div class="title-edit">
				...
				<xsl:if test="$required = 1">
					<xsl:call-template name="required.field" />
				</xsl:if>
			</div>
			<span>
				<input class="default" maxlength="255" type="text" name="{@input_name}" value="{.}" id="{generate-id()}">
	...
	<!-- Шаблон значения поля типа "Ссылка на язык" или "Ссылка на список языков" -->
	<xsl:template match="field[@type = 'lang_id' or @type = 'lang_id_list']" mode="form-modify">
		<xsl:param name="class">col-md-6</xsl:param>
		<div class="{$class} lang_field default-empty-validation">
			<div class="title-edit">
				<acronym title="{@tip}">
					<xsl:apply-templates select="." mode="sys-tips" />
					<xsl:value-of select="@title" />
			...
			<div class="layout-row-icon">
				<xsl:apply-templates select="value" mode="field-lang-id"/>
			</div>
		</div>
	</xsl:template>

	<!-- Шаблон списка языков поля типа "Ссылка на язык" или "Ссылка на список языков" -->
	<xsl:template match="value" mode="field-lang-id">
		<div class="layout-col-control selectize-container">
			<select class="default newselect" autocomplete="off" name="{../@input_name}">
				<xsl:if test="../type/@multiple = 'multiple'">
					<xsl:attribute name="multiple">multiple</xsl:attribute>
				...
				<xsl:apply-templates select="." mode="required_attr" />
				<option value=""/>
				<xsl:apply-templates select="lang" mode="field-lang-id"/>
			</select>
		</div>
	</xsl:template>

	<!-- Шаблон варианта значения поля типа "Ссылка на язык" или "Ссылка на список языков"-->
	<xsl:template match="lang" mode="field-lang-id">
		<option value="{@id}">
			<xsl:value-of select="." />
		</option>
	</xsl:template>

	<!-- Шаблон выбранного варианта значения поля типа "Ссылка на язык" или "Ссылка на список языков" -->
	<xsl:template match="lang[@selected = '1']" mode="field-lang-id">
		<option value="{@id}" selected="selected">
			<xsl:value-of select="." />
		</option>
	</xsl:template>

	<!-- Шаблон значения поля типа "Ссылка на тип цены" -->
	<xsl:template match="field[@type = 'price_type_id']" mode="form-modify">
		<xsl:param name="class">col-md-6</xsl:param>
		<div class="{$class} price_field default-empty-validation">
			<div class="title-edit">
				<acronym title="{@tip}">
					<xsl:apply-templates select="." mode="sys-tips" />
					<xsl:value-of select="@title" />
			...
			<div class="layout-row-icon">
				<xsl:apply-templates select="value" mode="field-price-type-id"/>
			</div>
		</div>
	</xsl:template>

	<!-- Шаблон списка языков поля типа "Ссылка на тип цены" -->
	<xsl:template match="value" mode="field-price-type-id">
		<div class="layout-col-control selectize-container">
			<select class="default newselect" autocomplete="off" name="{../@input_name}">
				<xsl:if test="../type/@multiple = 'multiple'">
					<xsl:attribute name="multiple">multiple</xsl:attribute>
				...
				<xsl:apply-templates select="." mode="required_attr" />
				<option value=""/>
				<xsl:apply-templates select="price" mode="field-price-type-id"/>
			</select>
		</div>
	</xsl:template>

	<!-- Шаблон варианта значения поля типа "Ссылка на тип цены"-->
	<xsl:template match="price" mode="field-price-type-id">
		<option value="{@id}">
			<xsl:value-of select="." />
		</option>
	</xsl:template>

	<!-- Шаблон выбранного варианта значения поля типа "Ссылка на тип цены" -->
	<xsl:template match="price[@selected = '1']" mode="field-price-type-id">
		<option value="{@id}" selected="selected">
			<xsl:value-of select="." />
		</option>
	</xsl:template>
	...
	<!-- Шаблон поля типа "Набор файлов" -->
	<xsl:template match="field[@type = 'multiple_file']" mode="form-modify">
		<div class="col-md-6 multifile" id="{generate-id()}"
			 umi:field-type="{@type}"
			 umi:name="{@name}"
			 umi:folder-hash="{@folder-hash}">
			<xsl:attribute name="data-prefix">
			...
			<div id="fileControlContainer_{generate-id()}" class="mfile_wrapper ui-sortable">
				<input type="hidden" umi:field-type="{@type}" name="{@input_name}" value="" />
				<xsl:for-each select="values/value">
					<xsl:if test="@relative-path">
				...
				<xsl:if test="count($modules-menu/items/item[@name='stat'])">
					<div class="layout-col-icon">
						<a href="javascript:void('0');" id="link{generate-id()}" class="icon-action tagPicker">
							<img title="&label-tags-cloud;" alt="&label-tags-cloud;" height="13" src="/styles/skins/modern/design/img/tags.gif" />
						</a>
					</div>
				</xsl:if>
			</div>
	...
	<!-- Шаблон полей типов "Файл", "Видео" и "Флеш-ролик" -->
	...
	<xsl:template match="field[@type = 'optioned']" mode="form-modify">
		<div class="col-md-6 optioned" id="{generate-id()}" umi:type="float">
			<xsl:call-template name="std-optioned-control" />
		</div>
	</xsl:template>

	<xsl:template match="field[@type = 'optioned' and @name = 'stores_state']" mode="form-modify">
		<div class="col-md-6 optioned stores default-empty-validation" id="{generate-id()}" umi:type="int">
			<xsl:call-template name="std-optioned-control">
	...
	<!-- Поле удалено, шаблон оставлен для обратной совместимости -->
	...
	<!-- Шаблон метки обязательности поля -->
	<xsl:template name="required.field">
		<sup>
			<xsl:text>*</xsl:text>
		</sup>
	</xsl:template>
