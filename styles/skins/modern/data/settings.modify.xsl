...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- Шаблон настройки "Сервисный аккаунт" в Google OAuth -->
	<xsl:template match="option[@name = 'google-service-account']" mode="settings.modify">
		<div class="row">
			<div class="col-md-4">
				<div class="title-edit">
					<acronym title="&option-google-hint-label;" class="acr">
						<xsl:value-of select="@label" />
						<xsl:text>*</xsl:text>
					</acronym>
				</div>
			...
			<div class="col-md-4">
				<input type="text" class="default" name="{@name}" value="{value}" id="{@name}" required="required" />
			</div>
		</div>
	</xsl:template>

	<!-- Шаблон заполненной настройки "Приватный ключ" в Google OAuth -->
	<xsl:template match="option[@name = 'google-private-key' and @type = 'password']" mode="settings.modify">
		<div class="row">
			<div class="col-md-4">
				<div class="title-edit">
					<acronym title="&option-google-hint-label;" class="acr">
			...
			<div class="col-md-4">
				<input class="default" type="password" name="{@name}" value="********" id="{@name}" disabled="disabled" />
				<a href="/admin/{$module}/deleteGoogleOAuthPrivateKey/">&label-delete-google-private-key;</a>
			</div>
		</div>
	</xsl:template>

	<!-- Шаблон пустой настройки "Приватный ключ" в Google OAuth -->
	<xsl:template match="option[@name = 'google-private-key' and @type = 'text']" mode="settings.modify">
		<div class="row">
			<div class="col-md-12 wysiwyg-field default-empty-validation">
				...
				<span>
					<textarea name="{@name}" id="{@name}" required="required text">
						<xsl:value-of select="." />
					</textarea>
				</span>
			</div>
