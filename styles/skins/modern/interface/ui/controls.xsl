...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="ui-smc-table">
		...
		<!-- Скрыть кнопку быстрого импорта из CSV -->
		<xsl:param name="hide-csv-quick-import-button">0</xsl:param>
		<!-- Скрыть кнопку быстрого экспорта В CSV -->
		<xsl:param name="hide-csv-quick-export-button">0</xsl:param>
		...
		<script type="text/javascript">
			...
			<xsl:if test="$hide-csv-quick-import-button = 1">
				hideQuickCsvImportButton: true,
			</xsl:if>

			<xsl:if test="$hide-csv-quick-export-button = 1">
				hideQuickCsvExportButton: true,
			</xsl:if>
			...
			$(cont).on('contextmenu', function (event) {
			var el = event.target;
			...
			$(cont).on('mousedown', function(event) {
	...
	<xsl:template name="ui-smc-tree">
		...
		<xsl:param name="interface-lang" select="/result/@interface-lang" />
		...
		<script type="text/javascript">
			...
			var oFilterController = new filterController(oTree, 'root-pages-type', true, null, {nativeAdvancedMode:true, interfaceLang:'<xsl:value-of select="$interface-lang"/>'}, searchStorage);
	...
	<xsl:template match="@demo" mode="stopdoItInDemo">
		<script type="text/javascript">
			...
			jQuery(function() {
			jQuery('#<xsl:value-of select="../@module" />_<xsl:value-of select="../@method" />_form').off().on('submit', function(e){
			e.stopImmediatePropagation();
			disableInDemo();
			return false;
			});
			jQuery('.<xsl:value-of select="../@module" />_<xsl:value-of select="../@method" />_btn').off().on('click', function(e){
			e.stopImmediatePropagation();
			disableInDemo();
			return false;
			});
