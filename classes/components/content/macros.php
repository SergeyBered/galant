<?php
	...
	class ContentMacros {
		...
		/**
		 * Устанавливает статус 403 и возвращает содержимое
		 * отсутствующей страницы
		 * @param string $template имя шаблона для tpl шаблонизатора
		 * @return mixed
		 * @throws ErrorException
		 * @throws coreException
		 */
		public function gen403($template = 'default') {
			if (!$template) {
				$template = 'default';
			}
			...
			$this->module->setHeader('%content_error_403_header%');
			list($template) = content::loadTemplates('content/forbidden/' . $template, 'block');
			return content::parseTemplate($template, []);
		}
...
<script src="/styles/common/js/cms/jquery.compiled.min.js?{$revision}" charset="utf-8"></script>
<script src="/styles/common/js/guest.js?{$revision}" charset="utf-8"></script>
...
<link rel="stylesheet" href="/styles/common/js/cms/panel/design.css?{$revision}" type="text/css"></link>
<link rel="stylesheet" href="/styles/skins/_eip/css/theme.css?{$revision}" type="text/css"></link>
<link rel="stylesheet" href="/styles/common/js/cms/eip/design.css?{$revision}" type="text/css"></link>

<script src="/styles/common/js/cms/panel/fakeUser.js?{$revision}" charset="utf-8"></script>
...
HTML;
		...
		public function getEditInPlaceFrontendResources() {
			...
			if ($eipWysiwygVersion === 'tinymce47') {
				$tinymceDirectory = '/styles/common/js/node_modules/tinymce';
				$tinymceCustomDirectory = "/styles/common/js/cms/wysiwyg/{$eipWysiwygVersion}";
			} else {
				$tinymceDirectory = "/js/cms/wysiwyg/{$eipWysiwygVersion}";
				$tinymceCustomDirectory = "/js/cms/wysiwyg/{$eipWysiwygVersion}";
			}

			return <<<HTML
<link type="text/css" rel="stylesheet" href="/styles/common/js/cms/compiled.min.css?{$revision}" />
{$this->getEditInPlaceThemeLink()}

<script>
	// Эту переменную нужно объявить ДО загрузки скрипта /js/cms/compiled.min.js,
...
<script src="/ulang/common.js?{$revision}" charset="utf-8"></script>
<script src="/styles/common/js/cms/jquery.compiled.min.js?{$revision}" charset="utf-8"></script>
<script src="{$tinymceDirectory}/tinymce.min.js?{$revision}" charset="utf-8"></script>
<script src="{$tinymceCustomDirectory}/tinymce_custom.js?{$revision}" charset="utf-8"></script>
<script src="/styles/common/js/cms/compiled.min.js?{$revision}" charset="utf-8"></script>
...
<script src="/styles/common/js/cms/jquery.compiled.min.js?{$revision}" charset="utf-8"></script>
<script src="/styles/common/js/guest.js?{$revision}" charset="utf-8"></script>
<link type="text/css" rel="stylesheet" href="/styles/common/js/node_modules/@fancyapps/fancybox/dist/jquery.fancybox.min.css?{$revision}" />
HTML;
		}
