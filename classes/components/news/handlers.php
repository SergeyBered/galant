<?php
	...
	class NewsHandlers {
		...
		/**
		 * Удаляет пробелы из начала и конца полей "Название" и "URL" ленты rss
		 * @param iUmiEventPoint $event событие сохранения rss
		 * @return bool
		 * @throws coreException
		 */
		public function fixRssSettings(iUmiEventPoint $event) {
			if ($event->getMode() != 'after') {
				return false;
			}
			...
			$rssTypeId = $typesCollection->getTypeIdByGUID('12c6fc06c99a462375eeb3f43dfd832b08ca9e17');
			...
			$object->setName(trim($name));
			$object->setValue('url', trim($url));
