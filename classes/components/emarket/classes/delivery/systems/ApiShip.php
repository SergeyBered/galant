<?php
	...
	class ApiShipDelivery extends delivery {
		...
		/** @const string DELIVERY_INCLUDE_FEES имя поля с опцией включения сборов в цену доставки */
		const DELIVERY_INCLUDE_FEES = 'include_fees';
		...
		/** @const string API_KEY_YANDEX_MAP имя поля с API ключом Яндекс.Карт */
		const API_KEY_YANDEX_MAP = 'yandex_map_api_key';
		...
		/**
		 * Возвращает включать ли в цену страховой сбор и сбор за кассовое обслуживание
		 * @return bool
		 */
		public function getSavedIncludeFees() : bool {
			 return (bool) $this->getObject()
				->getValue(self::DELIVERY_INCLUDE_FEES);
		}
		...
		public function getDeliveryOptionList(order $order) {
			try {
				$request = ApiShip\RequestDataFactory::createCalculateDeliveryCost(
					$this->getStoreCity(), $order, $this, $this->getStoreCountryCode()
				);
				return $this->getRequestSender()
					->calculate($request);
			} catch (Exception $e) {
		...
		public function initRequestSender($login, $password, $isDevMode = false, $keepLog = false) {
			...
			$requestSender = new ApiShip\RequestSender($login, $password, $isDevMode, $keepLog, $loggerFactory, $configuration);
		...
		/**
		 * Возвращает API Key Яндекс.Карт для работы виджета
		 * @return string
		 */
		public function getSavedApiKeyYandexMap() {
			return (string) $this->getObject()
				->getValue(self::API_KEY_YANDEX_MAP);
		}
		...
		protected function getStoreCity() {
			return (string) $this->getModule()
				->getSettings()
				->get(Settings::DEFAULT_STORE_SECTION, 'city');
		}

		/**
		 * Возвращает код страны склада
		 * @return string
		 * @throws publicAdminException
		 */
		protected function getStoreCountryCode() {
			return (string) $this->getModule()
				->getSettings()
				->get(Settings::DEFAULT_STORE_SECTION, 'country-code');
		}
