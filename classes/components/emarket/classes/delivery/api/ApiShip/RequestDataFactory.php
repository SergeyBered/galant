<?php
	...
	class RequestDataFactory implements iRequestDataFactory {
		...
		public static function createCalculateDeliveryCost($senderCity, \order $order, \ApiShipDelivery $delivery, $senderCountryCode) {
			$storeCity = self::createCity($senderCity, $senderCountryCode);
			...
			$customerCountryCode = $deliveryAddress->getCountryISOCode();
			$customerCity = self::createCity($customerCity, $customerCountryCode);
			...
			return new CalculateDeliveryCost(
				[
					...
					CalculateDeliveryCost::INCLUDE_FEES_KEY => $delivery->getSavedIncludeFees()
				]
			);
		}
		 ...
		 * @param Settings $setting настройки доставки модуля "Интерет-магазин"
		 * @return RequestDataParts\iDeliveryAgent
		 */
		private static function createOrderSender(Settings $setting) {
		...
		private static function createOrderItems(\order $order, iProvider $provider) {
			...
			foreach ($order->getItems() as $orderItem) {
				$orderItemsData[] = self::createOrderItem($order, $orderItem, $willBePayedByCOD, $weightRequired);
			}
		 ...
		 * @throws \publicAdminException
		 */
		private static function createOrderItem(\order $order, \orderItem $orderItem, $willBePayedByCOD, $weightRequired) {
			...
			$price = self::getOrderItemPrice($order, $orderItem);
			...
			return new RequestDataParts\OrderItem([
				...
				RequestDataParts\OrderItem::COST_KEY => (float) self::getOrderItemCost($price, $willBePayedByCOD),
				RequestDataParts\OrderItem::ASSESSED_COST_KEY => (float) $price
			]);
		}
		 ...
		 * @return float|string
		 */
		private static function getOrderItemPrice(\order $order, \orderItem $orderItem) {
		 ...
		 */
		private static function createCity($cityName, $countryCode) {
			return new RequestDataParts\City(
				[
					RequestDataParts\City::NAME_KEY => $cityName,
					RequestDataParts\City::COUNTRY_CODE => $countryCode
				]
			);
		}
		 ...
		 * @param Settings $setting настройки доставки модуля "Интернет-магазин"
		 * @param string $option название атрибута
		 * @return mixed
		 */
		private static function getStoreAttribute(Settings $setting, $option) {
			return $setting->get(Settings::DEFAULT_STORE_SECTION, $option);
		}
