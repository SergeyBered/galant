<?php
	...
	class EmarketMacros {
		...
		public function price($elementId = null, $template = 'default', $showAllCurrency = true) {
			...
			/** @var discount|itemDiscount|null  $discount */
		...
		public function handleBasketPut($itemType, $itemId, $order) {
			...
			$this->module->changeOrderItemAmount($order, $orderItem);
			$newAmount = $orderItem->getAmount();

			if ($newAmount <= 0) {
				$order->removeItem($orderItem);
			} elseif ($itemType == 'element') {
		...
		/**
		 * Изменяет количество наименования товара в корзине.
		 *
		 * @example:
		 * 1) /emarket/basket/put/element/16/ - увеличивает количество товара в корзине на единицу
		 * 2) /emarket/basket/put/element/16/?amount=2 - устанавливает количество товара в корзине равное двум
		 * 3) /emarket/basket/put/element/16/?append=2 - увеличивает количество товара в корзине на два
		 * 4) /emarket/basket/put/element/16/?reduce=2 - уменьшает количество товара в корзине на два
		 *
		 * @param order $order заказ (корзина)
		 * @param orderItem $item наименование товара
		 */
		public function changeOrderItemAmount(order $order, orderItem $item) {
			$module = $this->module;
			$previousAmount = $item->getAmount();

			switch (true) {
				case isset($_REQUEST['append']) : {
					$amountToAppend = getRequest('append');
					$module->validateAmount($amountToAppend);
					$amount = $previousAmount + $amountToAppend;
					break;
				}
				case isset($_REQUEST['reduce']) : {
					$amountToReduce = getRequest('reduce');
					$module->validateAmount($amountToReduce);
					$amount = $previousAmount - $amountToReduce;
					break;
				}
				default : {
					$amount = getRequest('amount');
					$module->validateAmount($amount);
					$amount = $module->getBasketPutAmount($order, $item);
				}
			}
		...
		protected function createNewOrderItemByBasketRequest($productId) {
			$tradeOfferId = $this->getTradeOfferIdByBasketRequest();
			$optionList = $this->getOptionListByBasketRequest();
			$priceTypeId = $this->getTradeOfferPriceTypeIdByBasketRequest();

			switch (true) {
				case $tradeOfferId : {
					return orderItem::createTradeOffer($productId, $tradeOfferId, $priceTypeId);
				}
				case is_array($optionList) : {
		...
		/**
		 * Возвращает идентификатор типа цены торгового предложения по параметрам запроса корзины
		 * @return int
		 */
		protected function getTradeOfferPriceTypeIdByBasketRequest() : int {
			return (int) getRequest('price_type_id');
		}
		 ...
		 * @param itemDiscount|null $discount скидка
		 * @return array
		 * @throws coreException
		 */
		...
		public function ordersList($template = 'default', $sort = 'asc', $ignorePaging = true, $limit = 50) {
			...
			$currentPage = $ignorePaging ? 0 : Service::Request()->pageNumber();
		 ...
		 * @param string $template
		 * @return mixed
		 * @throws ErrorException|coreException
		 */
		...
		public function gateway() {
			try {
				...
				if (defined('DEBUG') && DEBUG) {
					$buffer->push($e->getTraceAsString());
				}
		...
		/**
		 * Сохраняет промокод
		 * Если промокод не передан - берет его из запроса
		 ...
		 * @throws publicException
		 */
		public function savePromoCode(string $promoCode = null) : bool {
			if (!isset($promoCode)) {
				$promoCode = getRequest('promocode');
			}

			if (!$this->module->isPromoCodesAvailable()) {
				throw new publicException(getLabel('error-label-promocode-discounts-are-not-exists'));
			}

			if (!is_string($promoCode)) {
				throw new publicException(getLabel('error-label-promocode-not-a-string'));
			}

			if (isEmptyString($promoCode)) {
				throw new publicException(getLabel('error-label-promocode-string-is-empty'));
			}

			$order = $this->module->getBasketOrder();
			$cookieJar = Service::CookieJar();

			$cookieJar->setEncrypted('PromoCode', $promoCode);
			$order->refresh();

			if (!$this->isSavedPromoCodeApplied()) {
				$cookieJar->remove('PromoCode');
				throw new publicException(getLabel('error-label-promocode-wrong'));
			}
		...
		/**
		 * Возвращает сохраненный промокод
		 * @return string
		 */
		public function getSavedPromoCode() : string {
			$promoCode = Service::CookieJar()->getDecrypted('PromoCode');
			return (isset($promoCode)) ? $promoCode : '';
		}

		/**
		 * Определяет, применена ли к заказу (товару в заказе), который является корзиной, скидка по сохраненному промокоду
		 * @return bool
		 * @throws coreException
		 */
		private function isSavedPromoCodeApplied() : bool {
			$promoCode = $this->getSavedPromoCode();
			...
			if ($orderDiscount && $this->isSavedPromoCodeEqualsDiscountRules($orderDiscount)) {
				return true;
			}
			...
			foreach ($orderItems as $orderItem) {
				...
				if ($discount && $this->isSavedPromoCodeEqualsDiscountRules($discount)) {
					return true;
				}
			}
		...
		/**
		 * Определяет, соответствует ли сохраненный промокод правилам скидки
		 * @param discount $discount скидка
		 * @return bool
		 */
		private function isSavedPromoCodeEqualsDiscountRules(discount $discount) : bool {
			$promoCode = $this->getSavedPromoCode();
			$rules = $discount->getDiscountRules();

			foreach ($rules as $rule) {
				if ($rule instanceof promoCodeDiscountRule && $rule->getValue('promocode') == $promoCode) {
					return true;
				}
			}
		 ...
		 * @throws Exception
		 */
		private function handlePaymentRequest() : void {
			...
			/** @var payment $payment */
			$payment = payment::get($paymentId, $order);
			$this->logRequest($payment);

			/** @var iOutputBuffer $buffer */
			$buffer = $payment->poll();
			$this->logResponse($payment, $buffer);
		...
		/**
		 * Логгирует запрос от платежной системы
		 * @param payment $payment способ оплаты
		 * @return void
		 * @throws Exception
		 */
		private function logRequest(payment $payment) : void {
			$message = "Request:";

			$logger = $this->getLogger($payment);
			$logger->push($message, false);
			$logger->pushGlobalEnvironment()->save();
		}

		/**
		 * Логгирует ответ платежной системе
		 * @param payment $payment способ оплаты
		 * @param iOutputBuffer $buffer буффер
		 * @throws Exception
		 */
		private function logResponse(payment $payment, \iOutputBuffer $buffer) : void {
			$dump = $buffer->getDump();
			$message = "Response:\n" . $dump . "\n";
			$this->getLogger($payment)->log($message, false);
		}
		 ...
		 * @throws Exception
		 */
		private function getLogger(payment $payment) : iUmiLogger {
			$sysLogPath = mainConfiguration::getInstance()->includeParam('sys-log-path');
			$logPath = sprintf('%s/payment/%s/', rtrim($sysLogPath, '/'), str_replace('Payment', '', get_class($payment)));

			/** @var UmiCms\Utils\Logger\iFactory $loggerFactory */
			$loggerFactory = Service::get('LoggerFactory');
			return $loggerFactory->create($logPath);
		}
	}
