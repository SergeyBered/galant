<?php
	...
	class users extends def_module {
		...
		public function getLoginzaProvider() {
			...
			$result ['widget_url'] = $loginzaAPI->getWidgetUrl() . $providers;
		...
		public function createAuthorUser($user_id) {
			...
			$event = Service::EventPointFactory()
				->create('author_user_create', 'before')
				->setParam('author_user_id', $user_id)
				->setParam('author_name', $user_name)
				->setParam('author_type_id', $object_type_id);
			$event->call();

			$author_id = $objects->addObject($event->getParam('author_name'), $event->getParam('author_type_id'));
			$author = $objects->getObject($author_id);
			$author->is_registrated = true;
			$author->user_id = $event->getParam('author_user_id');
			$author->commit();

			$event->setMode('after');
			$event->setParam('author_id', $author->getId());
			$event->call();
		...
		public function createAuthorGuest($nick, $email, $ip) {
			...
			$event = Service::EventPointFactory()
				->create('author_guest_create', 'before')
				->setParam('author_name', $user_name)
				->setParam('author_nick_name', $nick)
				->setParam('author_email', $email)
				->setParam('author_ip', $ip)
				->setParam('author_type_id', $object_type_id);
			$event->call();

			$author_id = $objects->addObject($event->getParam('author_name'), $event->getParam('author_type_id'));
			$author = $objects->getObject($author_id);
			$author->setName($event->getParam('author_name'));
			$author->is_registrated = false;
			$author->nickname = $event->getParam('author_nick_name');
			$author->email = $event->getParam('author_email');
			$author->ip = $event->getParam('author_ip');
			$author->commit();
