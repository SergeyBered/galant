<?php
	...
	class DataMacros {
		...
		public function rss($elementId = null, $typeId = null) {
			...
			$xslPath = 'styles/common/xsl/rss.xsl';
		...
		public function atom($elementId = null, $typeId = null) {
			...
			$xslPath = 'styles/common/xsl/atom.xsl';
			/** @var DataMacros|DataFeeds $module */
			$module = $this->module;
			return $module->generateFeed($elementId, $xslPath, $typeId);
		}
		...
		private function renderProperty(iUmiObjectProperty $property, $template, $isRandom = false) {
			switch ($property->getDataType()) {
				...
				case 'multiple_file': {
					return $this->renderMultipleFiles($property, $template);
				}
				case 'relation': {
		...
		private function renderMultipleImageFiles(iUmiObjectProperty $property, $template) {
			...
			foreach ($value as $image) {
				$imageInfo = $this->getMultipleFileCommonInfo($image);
				$imageInfo['img_title'] = $image->getTitle();
				$imageInfo['img_alt'] = $image->getAlt();
				$imagesList[] = data::parseTemplate($imageBlock, $imageInfo);
			}

			$propertyInfo = $this->getMultipleFilePropertyInfo($property, $template, $imagesList);
			return data::parseTemplate($baseBlock, $propertyInfo);
		}

		/**
		 * Загружает и применяет шаблон для поля типа "Набор файлов"
		 * Блоки TPL-шаблона:
		 * multiple_files - основной блок полей типа "Набор файлов"
		 * multiple_files_empty - если в поле не содержатся данные
		 * multiple_files_item - блок для каждого отдельного изображения
		 *
		 * @param iUmiObjectProperty $property обрабатываемое свойство
		 * @param string $template имя шаблона
		 * @return mixed
		 */
		private function renderMultipleFiles(iUmiObjectProperty $property, $template) {
			list($baseBlock, $emptyBlock, $imageBlock) = data::loadTemplates(
				'data/' . $template,
				'multiple_files',
				'multiple_files_empty',
				'multiple_files_item'
			);
			...
			foreach ($value as $file) {
				$fileInfo = $this->getMultipleFileCommonInfo($file);
				$fileInfo['file_title'] = $file->getTitle();
				$fileList[] = data::parseTemplate($imageBlock, $fileInfo);
			}

			$propertyInfo = $this->getMultipleFilePropertyInfo($property, $template, $fileList);
			return data::parseTemplate($baseBlock, $propertyInfo);
		}

		/**
		 * Возвращает список с базовой информацией о файле
		 * @param iUmiFile $file файл
		 * @return array
		 */
		private function getMultipleFileCommonInfo(iUmiFile $file) {
			return [
				'size' => $file->getSize(),
				'filename' => $file->getFileName(),
				'filepath' => $file->getFilePath(),
				'src' => $file->getFilePath(true),
				'ext' => $file->getExt(),
				'title' => $file->getTitle(),
			];
		}
		...
		/**
		 * Возвращает список со свойствами объекта файла
		 * @param iUmiObjectProperty $property обрабатываемое свойство
		 * @param string $template имя шаблона
		 * @param array $fileList список информации о файлах
		 * @return array
		 */
		private function getMultipleFilePropertyInfo(iUmiObjectProperty $property, $template, array $fileList) {
			return [
				'field_id' => $property->getField()->getId(),
				'name' => $property->getName(),
				'title' => $property->getTitle(),
