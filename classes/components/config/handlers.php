<?php
	...
	class ConfigHandlers {
		...
		/**
		 * Ведет лог записей об обработке ответа с ошибкой
		 * @param iUmiEventPoint $event событие отправки буфера
		 * @throws Exception
		 */
		public function logResponseErrorEntry(iUmiEventPoint $event) {
			$response = Service::Response();
			$request = Service::Request();

			if ($response->isCorrect() || $request->isAdmin() || $event->getMode() !== 'before') {
				return;
			}

			try {
				Service::ResponseErrorEntryFacade()->log(
					$request->uri(),
					$response->getCurrentBuffer()->getStatusCode()
				);
