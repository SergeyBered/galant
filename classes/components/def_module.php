<?php
	...
	abstract class def_module {
		...
		public function getSortedModulesList() {
			...
			$sysModules = ['config', 'filemanager', 'backup', 'autoupdate', 'data', 'umiStub'];
			$utilModules = ['umiRedirects', 'umiNotifications', 'umiSettings', 'search', 'umiTemplates', 'trash'];
		...
		public function getFirstAllowedTabName() {
			...
			foreach ($commonTabList as $tabName => $methodAliases) {
				if (!$permissions->isAllowedMethod($userId, $moduleName, $tabName)) {
					continue;
				}
		...
		public function loadTemplateCustoms() {
			$template = cmsController::getInstance()
				->getCurrentTemplate();
			$customsDirectory = ($template instanceof iTemplate) ? $template->getCustomsDirectory() : '';

			if (!is_dir($customsDirectory)) {
				return;
			}

			$includesFile = realpath($customsDirectory) . '/' . get_class($this) . '/class.php';
			...
			if (endsWith(rtrim($customsDirectory, '/'), 'components')) {
				$className = ucfirst(get_class($this)) . 'Custom';
			} else {
		...
		/**
		 * Производит удаление модуля
		 * @param string|null $className если не задано - возьмет текущий
		 */
		public function uninstall(string $className = null) {
			$className = $className ?: get_class($this);
			$registry = Service::Registry();
			$defaultModuleAdmin = $registry->get('//settings/default_module_admin');

			if ($defaultModuleAdmin == $className) {
				$registry->set('//settings/default_module_admin', 'content');
			}

			$registry->delete('//modules/' . $className);
		}
		...
		public static function requireSlashEnding() {
			...
			if (Service::Request()->isXml() || umiCount($_POST) > 0) {
				return;
			}

			if (Service::Request()->isJson() || umiCount($_POST) > 0) {
				return;
			}
		...
		public function formatMessage($message, $bSplitLongMode = 0) {
			...
			if (preg_match_all("/\[smile:([^\]]+)\]/im", $message, $out)) {
				foreach ($out[1] as $smile_path) {
					$s = $smile_path;
					$smile_path = 'styles/common/images/forum/smiles/' . $smile_path . '.gif';
		...
		public function errorPanic() {
			...
			if (self::$noRedirectOnPanic || !Service::Request()->isHtml()) {
				$errorMessage = $this->getPanicErrorMessage();
		...
		/**
		 * Возвращает записаные ранее сообщения об ошибке
		 * @return string сообщения об ошибке
		 */
		public function getPanicErrorMessage() : string {
			$cmsController = cmsController::getInstance();
		...
		public function validateDeletingListPermissions() {
			$userId = Service::Auth()->getUserId();
			$isAllowedMethod = permissionsCollection::getInstance()
				->isAllowedMethod($userId, get_class($this),'delete');

			if (!$isAllowedMethod && isset($_REQUEST['dels'])) {
		...
		public function getIconPath() {
			$name = get_class($this);
			$path = "/styles/skins/modern/data/modules/{$name}/icon/{$name}.png";

			return file_exists('.' . $path) ? $path : "/images/cms/admin/modern/icon/{$name}.png";
		}

		/**
		 * Возвращает путь к файлу справки, работает только в административном режиме
		 * @return bool|string
		 * @throws coreException
		 */
		public function getManualUrl() {
			...
			if ($module instanceof def_module) {
				$extension = $module->getCurrentEntityHierarchicalTypeExtension();
			}

			return $this->getManualPath($methodName, $extension);
		}

		/**
		 * Возвращает путь к файлу справки
		 * @param string $methodName имя текущего метода административной панели
		 * @param string $extension расширение иерархического типа сущности
		 * @return string
		 */
		protected function getManualPath($methodName, $extension) {
			$path = false;

			if ($extension) {
				$manualFileName = "$methodName.$extension.html";
				$path = $this->getManualPathByFileName($manualFileName);
			}

			if (!$path) {
				$manualFileName = "$methodName.html";
				$path = $this->getManualPathByFileName($manualFileName);
			}

			return $path ?: ('./styles/common/man/' . ulangStream::getLangPrefix() . '/dummy.html');
		}

		/**
		 * Возвращает путь к файлу справки по имени файла
		 * @param string $fileName имя файла
		 * @return bool|string
		 */
		protected function getManualPathByFileName($fileName) {
			$moduleName = get_class($this);
			$manualSubDirectory = "$moduleName/man/" . ulangStream::getLangPrefix() . '/';
			$manualPath = './styles/skins/modern/data/modules/' . $manualSubDirectory . $fileName;

			if (is_file($manualPath)) {
				return $manualPath;
			}

			$manualPath = './' . $manualSubDirectory . $fileName;

			return is_file($manualPath) ? $manualPath : false;
		}
