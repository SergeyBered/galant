<?php
	...
	class CatalogMacros {
		...
		public function getCategoriesSelector($rootIdList, $level, $ignorePaging, $limit) {
			...
			$pageNumber = $ignorePaging ? 0 : Service::Request()->pageNumber();
		...
		public function getSmartCatalog(
			...
			if (Service::Registry()->get('//modules/catalog/disable_missing_products')) {
				$products->where('common_quantity')->more(0);
			}
			...
			return $this->getCatalogResponse($pages, $total, $limit, $itemTemplate, $itemsTemplate, $categoryId);
		}

		/**
		 * Возвращает ответ на запрос каталога товаров
		 * @param iUmiHierarchyElement[] $productList список товаров
		 * @param int $total общее количество товаров
		 * @param int $limit ограничение на общее количество товаров
		 * @param mixed $itemTemplate шаблон отображения товара (только для tpl)
		 * @param mixed $itemListTemplate шаблон отображения списка товаров(только для tpl)
		 * @param mixed $categoryId идентификатор категории товаров
		 * @return mixed
		 * @throws ErrorException
		 * @throws coreException
		 */
		public function getCatalogResponse(array $productList, $total, $limit, $itemTemplate = null, $itemListTemplate = null, $categoryId = null) {
			$result = [];
			$items = [];
			$umiLinksHelper = umiLinksHelper::getInstance();
			$umiHierarchy = umiHierarchy::getInstance();
			...
			foreach ($productList as $product) {
				$item = [];
				$pageId = $product->getId();
				$item['attribute:id'] = $pageId;
				$item['attribute:alt_name'] = $product->getAltName();
				$item['attribute:price'] = $product->getValue('price');
				$item['attribute:link'] = $umiLinksHelper->getLinkByParts($product);
				$item['xlink:href'] = 'upage://' . $pageId;
				$item['node:text'] = $product->getName();
			...
			return catalog::parseTemplate($itemListTemplate, $result, $categoryId);
		}
		 ...
		 * @throws selectorException
		 */
		public function getProductsByFlag($template = 'default', $flag, $limit) {
			$template = $template ?: 'default';

			list($itemListTemplate, $emptyItemListTemplate, $itemTemplate) = catalog::loadTemplates(
				'catalog/' . $template,
				'objects_block',
				'objects_block_empty',
				'objects_block_line'
			...
			if ($total == 0) {
				return $module->makeEmptyCatalogResponse($emptyItemListTemplate);
			}

			return $this->getCatalogResponse($pages, $total, $limit, $itemTemplate, $itemListTemplate);
		}

		/**
		 * Возвращает ответ на запрос пустого каталога товаров
		 * @param string $emptyItemsTemplate шаблон отображения (только для tpl)
		 * @param mixed $categoryId ид раздела каталога
		 * @return mixed
		 * @throws coreException
		 * @throws ErrorException
		 */
		public function makeEmptyCatalogResponse($emptyItemsTemplate, $categoryId = null) {
			$item = [];
