<?php
	...
	class Admin implements \iModulePart {
		...
		private function getOfferCollection(\iUmiHierarchyElement $page, $fieldName) {
			...
			$collection = Service::TradeOfferFacade()
				->getCollectionByIdList($offerListId);

			return $this->filterOfferCollectionByPageObjectTypeId($collection, $page->getObjectTypeId());
		}

		/**
		 * Возвращает коллекцию торговых предложений товара, в которой присутствуют только те торговые предложения,
		 * тип которых совпадает с типом объекта товара
		 * @param iOfferCollection $collection коллекция торговых предложений
		 * @param int $pageObjectTypeId id типа объекта страницы товара
		 * @return iOfferCollection
		 */
		private function filterOfferCollectionByPageObjectTypeId(iOfferCollection $collection, int $pageObjectTypeId) : iOfferCollection {
			$offerList = $collection->getList();

			foreach ($offerList as $offer) {
				if ($offer->getTypeId() != $pageObjectTypeId) {
					$collection->pull($offer->getId());
				}
			}
