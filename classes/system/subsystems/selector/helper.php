<?php
	...
	class selectorHelper {
		...
		/**
		 * Применяет к селектору условие фильтрации по параметру из $_REQUEST
		 * @param selector $selector селектор
		 * @param string $whereKey название поля, по которому производится фильтрация
		 * @param string $requestKey название поля из $_REQUEST с данными для фильтрации
		 * @return void
		 * @throws selectorException
		 */
		public static function detectWhereConditionByRequest(selector $selector, string $whereKey, string $requestKey) : void {
			$requestValue = (array) getRequest($requestKey);

			if (!empty($requestValue)) {
				$selector->where($whereKey)->equals($requestValue);
			}
		}
	}
