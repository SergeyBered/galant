<?php
	...
	class umiTemplaterPHP extends umiTemplater implements IFullResult {
		...
		public function parse($variables, $content = null) {
			...
			return $this->applyTemplate($cleanVariables, $this->getTemplatesSource());
		}
		 ...
		 * @throws ErrorException
		 */
		private function parseTplMacros($buffer, int $elementId = null) {
			if (!contains($buffer, '%')) {
				return $buffer;
			}

			$tplTemplate = umiTemplater::create('TPL');
			$elementId = $elementId ?: cmsController::getInstance()
				->getCurrentElementId();
