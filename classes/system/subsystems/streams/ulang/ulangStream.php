<?php
	...
	class ulangStream extends umiBaseStream {
		...
		private static function getModuleList($path) {
			...
			foreach (Service::Registry()->getList('//modules') as $registry) {
				$candidates[] = getArrayKey($registry, 0);
			}
		...
		private static function getI18nSources() {
			...
			try {
				$template = cmsController::getInstance()
					->getCurrentTemplate();
				$customsDirectory = ($template instanceof iTemplate) ? $template->getCustomsDirectory() : null;
			} catch (Exception $e) {
				return $sources;
			}
