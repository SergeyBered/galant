<?php
	...
	set_time_limit(0);
	$isGuiUpdate = isset($_REQUEST['guiUpdate']) && $_REQUEST['guiUpdate'] == 'true';

	if ((isset($_REQUEST['step']) && $_REQUEST['step'] == 'ping') && $isGuiUpdate) {
	...
	if (mb_strpos(__DIR__, 'classes/system/installer') !== false) {
		...
		if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == 'install') {
			...
			$installMode = false;
		} elseif ((isset($_REQUEST['mode']) && $_REQUEST['mode'] == 'update') || $isGuiUpdate) {
			$installMode = false;
			touch($controlFilePath);
	...
	class umiInstallExecutor {
		...
		/** @var bool isOverwritableStateSaved было ли сохранено перезаписываемое состояние  */
		static private $isOverwritableStateSaved = false;
		...
		private function saveSettings(array $new = []) {
			...
			self::$isOverwritableStateSaved = true;
			return true;
		}
		...
		private function isOverwritableStateSaved() {
			...
			return $login && ($password || $md5pass) && ($isUsingSsl !== null) && ($favicon !== null) && self::$isOverwritableStateSaved;
		}
		...
		private function copySpecialFiles() {
			// preload services files before reloading
			ServiceContainerFactory::create()->get('Response');
			ServiceContainerFactory::create()->get('Request');
			ServiceContainerFactory::create()->get('Auth');

			$specialDirectoryPathList = [
				"{$this->temporaryDirectory}/core/classes/vendor/",
				"{$this->temporaryDirectory}/core/classes/system/cache/",
				...
				"{$this->temporaryDirectory}/core/classes/system/subsystems/models/auth/",
			...
			$ignoredFiles = [
				"{$this->temporaryDirectory}/core/classes/system/autoload/autoload.custom.php"
			];
			...
			foreach ($specialDirectoryPathList as $specialDirectoryPath) {
				...
				foreach ($specialDirectory->getAllFiles($fileType) as $filePath => $fileName) {
					if (!$this->installMode && in_array($filePath, $ignoredFiles)) {
						continue;
					}
