<?php
	...
	class testHost {
		...
		private function testPhpVersion() {
			$check = version_compare(phpversion(), '7.2.5', '>') && version_compare(phpversion(), '7.5', '<');
		...
		private function isHttps() {
			if (isset($_SERVER['HTTPS']) && in_array($_SERVER['HTTPS'], array('ON', 'On', 'on', 1))) {
				return true;
			}
		...
		private function checkBasicDatabaseQueries() {
			...
			$this->assertQuery("insert into `test{$time}` (b) values (11)", 13110);
			$this->assertQuery("select * from `test{$time}`", 13111);
