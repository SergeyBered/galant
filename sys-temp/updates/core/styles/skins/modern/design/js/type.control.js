...
var modernTypeController = (function(jQuery) {
	...
	var initElements = function(scope) {
		...
		titleInput.on('focus', function(event) {
			$(this).trigger("select");

			if (!nameInput.val().length) {
				$(event.currentTarget).on('keyup', {nameField: nameInput}, universalTitleConvertCallback);
			}
		}).on('blur', function(event) {
			$(event.currentTarget).off('keyup', universalTitleConvertCallback);
		});
	...
	var drawFieldInsideGroup = function(groupId, fieldProperties) {
		...
		fieldLineView.find('a.fremove').on('click', function(e) {
	...
	var initFieldForm = function(options, scope) {
		$('#newtype', scope).add('#' + options.id + 'type', scope).on('change', function() {
	...
	var actor = {
		...
		init: function(id, gmodel) {
			...
			$('a.add_group').on('click', function(event) {
			...
			$('a.gedit').on('click', editGroupHandler);
			$('a.fedit').on('click', editFieldHandler);
			$('a.gremove').on('click', removeGroupHandler);

			$('a.fremove').on('click', function(e) {
				var id = $(e.target).parent().attr('data');
				removeField(id);
			});

			$('a.fadd').on('click', function(e) {
				var id = $(e.target).attr('data');
				addField(id);
			});

			$('input[name="data[title]"]').on('focus', function(event) {
				var nameInput = $(event.currentTarget).parent().parent().find('input[name="data[name]"]');
				$(this).on('keyup', {nameField: nameInput}, function(e) {
					universalTitleConvertCallback(e);
				});

			}).on('blur', function(event) {
				$(event.currentTarget).off('keyup');
			});
