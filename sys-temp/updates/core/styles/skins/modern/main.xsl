...
<xsl:stylesheet version="1.0" exclude-result-prefixes="xlink"
	...
	<xsl:variable name="domain-url" select="/result/@domain-url"/>
	...
	<xsl:variable name="request-url" select="/result/@request-url"/>

	<!-- Result data tag properties -->
	<xsl:variable name="result-data" select="/result/data"/>
	...
	<!-- Context manual url -->
	<xsl:variable name="context-manul-url" select="document(concat('udata://', $module, '/getManualUrl'))/udata" />
