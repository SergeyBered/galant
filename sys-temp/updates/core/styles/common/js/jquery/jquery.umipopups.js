...
(function(jQuery) {
			...
			for (var i = 0; i < openedPopups.length; i++) {
				if (openedPopups[i].name == name) {
					...
					if (focusableElement.length > 0) {
						jQuery(focusableElement[focusableElement.length-1]).trigger("focus");
					}
	...
	function showScreenLocker() {
		if (jQuery("#popupLayerScreenLocker").length) {
			...
			let query = '';
			if (openedPopups[openedPopups.length - 2]) {
				query = openedPopups[openedPopups.length - 2].name ? openedPopups[openedPopups.length - 2].name : '';
			}

			if (frameElement && frameElement.id != denyFrameId) {
				let openedPopupsZIndex = window.parent.jQuery("#popupLayer_" + query).css("z-index");
				let zIndex = parseInt(openedPopups.length == 1 ? 999 : openedPopupsZIndex) + 1;
				window.parent.jQuery('#popupLayerScreenLocker').css("z-index", zIndex.toString());
			} else {
				let openedPopupsZIndex = jQuery("#popupLayer_" + query).css("z-index");
				let zIndex = parseInt(openedPopups.length == 1 ? 999 : openedPopupsZIndex) + 1;
				jQuery('#popupLayerScreenLocker').css("z-index", zIndex.toString());
			}
		} else {
	...
	function hideScreenLocker(popupName) {
		...
		if (openedPopups.length > 0) {
			...
			let zIndex = parseInt(popupLayer.css("z-index")) - 1;
			popupLayerScreenLocker.css("z-index", zIndex.toString());
		}
	}
	...
	function showPopupLayerContent(popupObject, newElement, data) {
		...
		jQuery('.eip_win_head', popupElement).on('mousedown', function() {
			popupElement.draggable({containment: 'window'});
		});

		jQuery('.eip_win_body', popupElement).on('mousedown', function() {
			popupElement.draggable().draggable('destroy');
		});

		jQuery('.eip_win_close', popupElement).on('click', function() {
		...
		jQuery("#" + idElement + " .jmp-link-at-top, " +
		  "#" + idElement + " .jmp-link-at-bottom").on('focus', function(){
			jQuery(focusableElement[focusableElement.length-1]).trigger("focus");
		});
