...
(function(jQuery) {
	...
	jQuery.fn.jGrowl = function( m , o ) {
		if ( typeof this.each === "function" ) {
			...
			return this.each(function() {
				...
				if ( typeof jQuery(this).data('jGrowl.instance')[m] === "function" ) {
	...
	jQuery.extend( jQuery.fn.jGrowl.prototype , {
		...
		render: 		function( notification ) {
			...
			notification.find('div.jGrowl-close').on('click', o.close);

			/** Notification Actions **/
			jQuery(notification).on("mouseover.jGrowl", function() {
			...
			if ( jQuery('div.jGrowl-notification:parent', self.element).length > 1 && 
				 jQuery('div.jGrowl-closer', self.element).length == 0 && this.defaults.closer != false ) {
				// CUSTOM
		...
		update:	 function() {
			...
			if ( this.notifications.length > 0 && 
				 (this.defaults.pool == 0 || jQuery(this.element).find('div.jGrowl-notification:parent').length < this.defaults.pool) )
				this.render( this.notifications.shift() );

			if ( jQuery(this.element).find('div.jGrowl-notification:parent').length < 2 ) {
