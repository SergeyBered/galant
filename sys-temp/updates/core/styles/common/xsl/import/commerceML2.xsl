...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="Документ">
		...
		<xsl:param name="order_status_id" select="$properties[Наименование = 'Статуса заказа ИД']/Значение" />
		<xsl:param name="order_status_exists">
			<xsl:choose>
				<xsl:when test="$order_status_id">
					<xsl:value-of select="document(concat('udata://exchange/checkOrderStatusExistsById/', $order_status_id))/udata"/>
				</xsl:when>
		...
		<object id="{Номер}" update-only="1">
			<properties>
				<group name="order_props">
					...
					<xsl:if test="$order_status_exists = 1">
						<property name="status_id">
							<title>Статус заказа</title>
							<value><xsl:value-of select="$order_status_id" /></value>
						</property>
					</xsl:if>
				</group>
