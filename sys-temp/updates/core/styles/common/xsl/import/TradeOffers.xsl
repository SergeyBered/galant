...
<xsl:stylesheet version="1.0"
	...
	<xsl:key name="group" match="Предложение" use="substring-before(Ид, '#')"/>
	...
	<xsl:template match="ПакетПредложений" mode="TradeOffers">
		...
		<relations>
			<xsl:apply-templates 
				select="Предложения/Предложение[generate-id(.) = generate-id(key('group',substring-before(Ид, '#')))]" 
				mode="TradeOfferRelation"/>
		</relations>
	</xsl:template>
		...
		<type id="{$type.id}">
			...
			<fieldgroups>
				<group title="i18n::fields-group-trade-offers" name="trade_offers">
					...
					<field name="trade_offer_image_list" title="i18n::field-trade-offer-image-list" visible="visible">
						<type name="i18n::field-type-multiple-image" data-type="multiple_image" multiple="multiple"/>
					</field>
		...
		<object id="{$offer.id}" guid="{$offer.id}" name="{Наименование}" type-id="{$type.id}">
			<properties>
				<group title="i18n::fields-group-trade-offers" name="trade_offers">
					...
					<xsl:apply-templates select="Картинка[position() = 1]" mode="TradeOfferCharacteristic"/>

					<xsl:if test="Картинка[position() > 1]">
						<property name="trade_offer_image_list" type="multiple_image" multiple="multiple">
						<title>i18n::field-trade-offer-image-list</title>
							<xsl:for-each select="Картинка[position() > 1]">
								<value path="./images/cms/data/{.}" />
							</xsl:for-each>
						</property>
					</xsl:if>
				</group>
	...
	<xsl:template match="Цена" mode="TradeOfferPrices">
		...
		<xsl:variable name="currency.id" select="document(concat('udata://exchange/getCmlCurrencyIdByAlias/?alias=', php:function('urlencode', string($currency.code))))"/>
		<xsl:variable name="price.id" select="document(concat('udata://exchange/getCmlOfferPriceInternalId/', $currency.id, '/', ИдТипаЦены, '/', $offer.id))/udata"/>
		<entity id="" service="TradeOfferPriceExchange">
			<xsl:attribute name="id">
				<xsl:choose>
					<xsl:when test="string-length($price.id) > 0">
						<xsl:value-of select="$price.id" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="concat($offer.id, '#', ИдТипаЦены)" />
					</xsl:otherwise>
				</xsl:choose>
			</xsl:attribute>
			<offer_id>
			...
			<currency_id>
				<xsl:value-of select="$currency.id"/>
			</currency_id>
			<value>
	...
	<xsl:template match="Склад" mode="TradeStockBalance">
		<xsl:param name="offer.id"/>
		<xsl:variable name="stock.id" select="document(concat('udata://exchange/getCmlStockInternalId/', @ИдСклада))/udata"/>
		<entity>
			<xsl:attribute name="id">
				<xsl:choose>
					<xsl:when test="string-length($stock.id) > 0">
						<xsl:value-of select="$stock.id" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="concat($offer.id, '#', @ИдСклада)" />
					</xsl:otherwise>
				</xsl:choose>
			</xsl:attribute>
			<xsl:attribute name="service">
				<xsl:text>TradeStockBalanceExchange</xsl:text>
			</xsl:attribute>
			<offer_id>
				<xsl:value-of select="$offer.id"/>
			</offer_id>
	...
	<xsl:template match="Предложение[contains(Ид, '#')]" mode="TradeOfferRelation">
		...
		<relation page-id="{$page.id}" field-name="trade_offer_list">
			<xsl:apply-templates select="key('group',substring-before(Ид, '#'))" mode="TradeOfferRelationOffer"/>
		</relation>
	</xsl:template>
	...
	<xsl:include href="udata://core/includeExchangeXsl/import/TradeOffers" />
