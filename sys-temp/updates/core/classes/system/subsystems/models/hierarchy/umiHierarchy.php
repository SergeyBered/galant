...
SQL;
			...
			$offerIdTable = $schema->getOfferIdTable();
			$sql = <<<SQL
INSERT INTO {$offerIdTable}
(`obj_id`, `field_id`, `offer_id`)
    SELECT '{$newObjectId}', `field_id`, `offer_id`
        FROM {$offerIdTable}
            WHERE obj_id = '{$objectId}'
SQL;
			$connection->query($sql);
...
SQL;
			...
			if ($copySubPages) {
				$this->cloneElementChildren($id, $newElementId);
			}
		...
		public function cloneElementChildren($previousParentId, $nextParentId = false, $childrenTypeId = false) {
			$previousParent = $this->getElement($previousParentId);

			if (!$previousParent instanceof iUmiHierarchyElement) {
				throw new expectElementException(sprintf('Cannot get previous parent element by id: %s', $previousParentId));
			}

			$children = $this->getChildrenTree(
				$previousParent->getId(), true, true, 0, $childrenTypeId, $previousParent->getDomainId()
			);

			$nextParentId = $nextParentId ?: $previousParentId;
			$nextParent = $this->getElement($nextParentId);

			if (!$nextParent instanceof iUmiHierarchyElement) {
				throw new expectElementException(sprintf('Cannot get next parent element by id: %s', $nextParentId));
			}

			foreach ($children as $childId => $_dummy) {
				$cloneId = $this->cloneElement($childId, $nextParent->getId(), false);

				if ($cloneId) {
					$this->cloneElementChildren($childId, $cloneId, $childrenTypeId);
				}
			}
...
TOTALSQL;
		...
		public function getMaxDepth($rootPageId, $maxDepth) {
			...
			return $this->getLevel($rootPageId) + $maxDepth;
		}

		/** @inheritDoc */
		public function getLevel(int $pageId) : int {
			$sql = <<<SQL
SELECT `level` FROM `cms3_hierarchy_relations` WHERE `child_id` = $pageId LIMIT 0,1;
SQL;
			$result = ConnectionPool::getInstance()
				->getConnection()
				->queryResult($sql)
				->setFetchArray();
			return ($result->length() > 0) ? (int) $result->fetch()['level'] : 0;
		}
		...
		public function getAbsolutePath(int $id) : string {
			$oldMode = $this->forceAbsolutePath();
			$path = $this->getPathById($id);
			$this->forceAbsolutePath($oldMode);
			return $path;
		}
		...
		public function getPathById(
			...
			if (!$ignoreBaseType && $element->getHierarchyType()->hidePages()) {
				return $this->pathCache[$cachePath] = '';
			}
...
SQL;
			...
			if (!$ignoreBaseType) {
				$page = $this->getElement($id);

				if (!$page instanceof iUmiHierarchyElement || $page->getHierarchyType()->hidePages()) {
					return $this->idByPathCache[$elementHash] = false;
				}
 			}
...
SQL;
		...
		private function parseAltName($altName) {
			preg_match("/^([a-z0-9_.-]*)([1-9]\d*)?$/U", $altName, $matches);
