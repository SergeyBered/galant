<?php
	...
	class FilterIndexGenerator {
		...
		/**
		 * Возвращает значение полей типа "Выпадающий список", "Ссылка на домен", "Ссылка на список доменов", 
		 * "Ссылка на язык", "Сссылка на список языков" подготовленное к записи в индекс
			...
			switch (true) {
				case $isMultiple && is_array($value) && umiCount($value) > 0: {
					...
					foreach ($value as $val) {
						switch ($field->getDataType()) {
							case 'domain_id_list': {
								$valueList[] = Service::DomainCollection()->get($val)->getHost();
								break;
							}
							case 'lang_id_list': {
								$valueList[] = Service::LanguageCollection()->get($val)->getTitle();
								break;
							}
							case 'relation': {
								$valueList[] = $this->getObjectName($val);
								break;
							}
						}
					}
				...
				case !$isMultiple && is_numeric($value): {
					...
					switch ($field->getDataType()) {
						case 'domain_id': {
							$name = Service::DomainCollection()->get($value)->getHost();
							break;
						}
						case 'lang_id': {
							$name = Service::LanguageCollection()->get($value)->getTitle();
							break;
						}
						case 'relation': {
		...
		private function createTable() {
			...
			$columnsIndexes = array_slice($columnsIndexes, 0, (self::MAX_KEYS_COUNT - 1));
			$columnsIndexesSql = '';
...
SQL;
		...
		private function getEntities() {
			...
			if ($entitiesType == 'pages') {
				...
				if ((bool) Service::Registry()->get('//modules/catalog/disable_missing_products')) {
					$entities->where('common_quantity')->more(0);
				}
			}
		...
		private function loadFilteredFields() {
			...
			if (!is_array($savedFilteredFields) || array_diff($savedFilteredFields, $filteredFieldsIds) !== []) {
		...
		private function getObjectTypeIdsByHierarchy() {
			...
			$pages->where('lang')->isnotnull();
			$pages->where('domain')->isnotnull();
