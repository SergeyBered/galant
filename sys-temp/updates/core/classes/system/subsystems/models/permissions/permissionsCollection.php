<?php
	...
	use UmiCms\Service;
	use UmiCms\System\Module\Permissions\iLoader as PermissionsLoader;
...
SQL;
		...
		/** @inheritDoc */
		public function isAllowedAdminEntryPoint(int $userId, string $module, string $method) : bool {
			$entryPoints = Service::ModulePermissionLoader()
				->loadAdminEntryPoints($module);

			if (!in_array($method, $entryPoints)) {
				return false;
			}

			if (!$this->isAllowedModule($userId, $module)) {
				return false;
			}

			return $this->isAllowedMethod($userId, $module, $method);
		}

		/** @inheritDoc */
		public function getFirstAllowedAdminEntryPoint(int $userId, string $module = null) : array {
			/** @var PermissionsLoader $permissionsLoader */
			$permissionsLoader = Service::ModulePermissionLoader();

			if ($module) {
				$entryPoints = $permissionsLoader->loadAdminEntryPoints($module);

				foreach ($entryPoints as $entryPoint) {
					if ($this->isAllowedModule($userId, $module) && $this->isAllowedMethod($userId, $module, $entryPoint)) {
						return [
							'module' => $module,
							'method' => $entryPoint
						];
			...
			$modulesList = cmsController::getInstance()->getModulesList();
			...
			throw new privateException('Allowed admin entry points not found');
		}

		/** @inheritDoc */
		public function hasAllowedAdminEntryPoints(int $userId) : bool {
			$permissionsLoader = Service::ModulePermissionLoader();
			$modules = cmsController::getInstance()->getModulesList();

			foreach ($modules as $module) {
				$adminEntryPoints = $permissionsLoader->loadAdminEntryPoints($module);

				foreach ($adminEntryPoints as $adminEntryPoint) {
					if ($this->isAllowedAdminEntryPoint($userId, $module, $adminEntryPoint)) {
						return true;
					}
				}
			}
