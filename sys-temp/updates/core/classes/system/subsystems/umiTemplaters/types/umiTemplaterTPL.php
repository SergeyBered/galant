<?php
	...
	class umiTemplaterTPL extends umiTemplater {
		...
		protected $macrosWhiteList = [];

		/** @var string[] $macrosBlackList черный список макросов */
		protected $macrosBlackList = [];
		...
		/**
		 * Устанавливает белый список макросов
		 * @param string[] $macrosList белый список макросов
		 * @example [
		 * 		'content/redirect'
		 * ]
		 * @return $this
		 */
		public function setMacrosWhiteList(array $macrosList) : umiTemplaterTPL {
			$this->macrosWhiteList = $macrosList;
			return $this;
		}
		 ...
		 * @return $this
		 */
		public function setMacrosBlackList(array $macrosList) : umiTemplaterTPL {
			$this->macrosBlackList = $macrosList;
			return $this;
		}

		/**
		 * Проверяет, можно ли запускать макрос
		 * @param string $macrosURI макрос
		 * @example content/redirect
		 * @return bool
		 */
		private function isExecuteMacrosAllowed($macrosURI) {
			if (count($this->macrosWhiteList) === 0 && count($this->macrosBlackList) === 0) {
				return true;
			}

			if (count($this->macrosBlackList) > 0) {
				return in_array($macrosURI, $this->macrosBlackList) === false;
			}

			if (count($this->macrosWhiteList) > 0) {
				return in_array($macrosURI, $this->macrosWhiteList);
			}
		...
		protected function executeCompleteMacros($module, $method = null, $args = [], array $variables) {
			...
			if ($method === null && is_callable($module)) {
				...
				if ($controller->isVirtualModule($module)) {
					$moduleInst = system_buildin_load($module);
				} elseif (permissionsCollection::getInstance()->isAllowedMethod($userId, $module, $method)) {
		...
		public function executeOnlyAllowedMacroses($allowedMacroses) {
			$this->setMacrosWhiteList((array) $allowedMacroses);
		}
	}
