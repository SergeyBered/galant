...
/** Класс функционала административной панели */
	class AppointmentAdmin {
		...
		public function config() {
			...
			$config = [
				'appointment-notify-options' => [
					...
					"boolean:{$optionsList['notifications']['useCaptcha']}" => null
				],
			];
			...
			if ($this->isSaveMode()) {
				...
				$umiRegistry->set(
					"//modules/appointment/{$optionsList['notifications']['useCaptcha']}",
					$userValues['appointment-notify-options']["boolean:{$optionsList['notifications']['useCaptcha']}"]
				);
			...
			$params = [
				'appointment-notify-options' => [
					...
					"boolean:{$optionsList['notifications']['useCaptcha']}" => $umiRegistry->get(
						"//modules/appointment/{$optionsList['notifications']['useCaptcha']}"
					),
				],
			];
		...
		/**
		 * Копирует страницу онлайн-записи
		 * @throws Exception
		 * @throws coreException
		 * @throws expectElementException
		 * @throws publicAdminException
		...
		protected function getServiceConfig() {
			return [
				...
				'default' => 'name[250px]|time[250px]|price[150px]',
				'fields' => [
					[
						'name' => 'name',
