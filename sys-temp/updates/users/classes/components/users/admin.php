	...
	class UsersAdmin {
		...
		public function users_list() {
			...
			if (getRequest('param0') == 'outgroup') {
				$selector->where('groups')->isnull();
			} else {
				$groupId = $this->expectObjectId('param0');

				if ($groupId) {
					$selector->where('groups')->equals($groupId);
				}
			}
			...
			if (!$umiPermissions->isSv()) {
				$selector->where('id')->notequals(Service::SystemUsersPermissions()->getSvUserId());
			}

			$loginSearch = getRequest('search');

			if ($loginSearch) {
				$selector->where('login')->like('%' . $loginSearch . '%');
			}
		...
		public function groups_list() {
			...
			$selector = Service::SelectorFactory()
				->createObjectTypeName('users', 'users');

			$umiPermissions = permissionsCollection::getInstance();

			if (!$umiPermissions->isSv()) {
				$selector->where('id')->notequals(Service::SystemUsersPermissions()->getSvGroupId());
			}
		...
		public function authorsList() {
			...
			$selector = Service::SelectorFactory()
				->createObjectTypeName('users', 'author');
			$limit = $this->getLimit();
