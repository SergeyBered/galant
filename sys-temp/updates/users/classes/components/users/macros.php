<?php
	...
	class UsersMacros {
		...
		public function login_do() {
			...
			$user = $this->getUserByCredentials($login, $password);
			...
			try {
				...
				$user = umiObjectsCollection::getInstance()->getById($userId);

				if ($user instanceof iUmiObject && !$user->getValue('is_activated') && !$this->isDeactivatedByManager($user))  {
					return $this->handleLoginNotActivatedUser($login);
				}
		...
		/**
		 * Обрабатывает результат авторизации неактивированного пользователя
		 * @param string $login логин
		 * @return mixed
		 * @throws Exception
		 */
		public function handleLoginNotActivatedUser(string $login) {
			$this->triggerLoginNotActivatedUser($login);

			if (getRequest('redirect_disallow') == 1) {
				return $this->reactivate();
			}

			return $this->module->redirect($this->module->pre_lang . '/users/reactivate/');
		}

		/**
		 * Вызывает событие при попытке авторизации неактивированного пользователя
		 * @param string $login логин
		 * @return void
		 * @throws Exception
		 */
		public function triggerLoginNotActivatedUser(string $login) : void {
			Service::EventPointFactory()
				->create('users_login_not_activated_user')
				->setParam('login', $login)
				->call();
		}

		/**
		 * Выводит форму для повторной отправки письма с кодом активации
		 * @param string $template имя шаблона для tpl-шаблонизатора
		 * @return mixed
		 * @throws ErrorException
		 * @throws coreException
		 */
		public function reactivate($template = 'default') {
			list($template_block) = users::loadTemplates('users/register/' . $template, 'reactivate');
			return users::parseTemplate($template_block, ['not_activated' => true]);
		}

		/**
		 * Отправляет на почту письмо для переактивации пользователя
		 * @param string $template имя шаблона для tpl-шаблонизатора
		 * @return mixed
		 * @throws ErrorException
		 * @throws coreException
		 ...
		 */
		public function reactivate_do($template = 'default') {
			$module = $this->module;
			...
			$refererUrl = Service::Request()->referrer();

			if ($refererUrl == '') {
				$refererUrl = $module->pre_lang . '/users/reactivate/';
			}

			$module->errorRegisterFailPage($refererUrl);

			$login = (string) getRequest('reactivate_login');
			$email = (string) getRequest('reactivate_email');
			$reactivateByLogin = $login != '';
			$reactivateByEmail = $email != '';
			$userId = false;

			if ($reactivateByLogin) {
				try {
					$userId = $this->getUserIdByLogin($login);
			...
			if (!$userId) {
				if ($reactivateByLogin && !$reactivateByEmail) {
					$module->errorNewMessage('%errors_forget_wrong_login%');
				}

				if ($reactivateByEmail && !$reactivateByLogin) {
					$module->errorNewMessage('%errors_forget_wrong_email%');
				}

				if (($reactivateByEmail && $reactivateByLogin) || (!$reactivateByEmail && !$reactivateByLogin)) {
			...
			if ($user->getValue('is_activated')) {
				$module->errorNewMessage('%error_user_already_activated%');
				$module->errorPanic();
			}

			if ($this->isDeactivatedByManager($user)) {
				$module->errorNewMessage('%errors_forget_nonactivated_login%');
				$module->errorPanic();
			}

			$domain = Service::DomainDetector()->detect();
			$activationCode = $user->getValue('activate_code');
			$activationLink = $this->getLink($domain, $activationCode, 'activate');
			...
			if ($this->module->isUsingUmiNotifications()) {
				...
				$notification = $mailNotifications->getCurrentByName('notification-users-registered');

				if ($notification instanceof MailNotification) {
					$subjectTemplate = $notification->getTemplateByName('users-registered-subject');
					$contentTemplate = $notification->getTemplateByName('users-registered-content');
			...
			if ($subject && $content) {
				Service::EventPointFactory()
					->create('usersSendMail', 'before')
					->addRef('variables', $variables)
					->call();

				$mailSettings = $module->getMailSettings();
				$emailFrom = $mailSettings->getSenderEmail();
				$fioFrom = $mailSettings->getSenderName();
				$fio = $variables['lname'] . ' ' . $variables['fname'] . ' ' . $variables['father_name'];

				$mail = new umiMail();
				$mail->addRecipient($variables['email'], $fio);
				...
				Service::EventPointFactory()
					->create('usersSendMail', 'after')
					->addRef('variables', $variables)
					->call();
			}

			$module->redirect($module->pre_lang . "/users/reactivate_done");
		}

		/**
		 * Выводит информацию о результате отправки сообщения для переактивации
		 * @param string $template
		 * @return mixed
		 * @throws ErrorException
		 * @throws coreException
		 */
		public function reactivate_done($template = 'default') {
			list($template_block) = users::loadTemplates('users/register/' . $template, 'reactivate_done');
		...
		public function registrate_do() {
			...
			if (!$module->checkIsUniqueLogin($login)) {
				$userId = $this->getUserIdByLogin($login);
				$user = umiObjectsCollection::getInstance()->getById($userId);

				if (!$user->getValue('is_activated') && !$this->isDeactivatedByManager($user)) {
					$module->errorAddErrors([
						'message' => 'err_users_login_not_activated'
					]);
				}
			}

			if (!$module->checkIsUniqueEmail($email)) {
				$userId = $this->getUserIdByEmail($email);
				$user = umiObjectsCollection::getInstance()->getById($userId);

				if (!$user->getValue('is_activated') && !$this->isDeactivatedByManager($user)) {
					$module->errorAddErrors([
						'message' => 'err_users_email_not_activated'
					]);
				}
			}
		...
		/**
		 * Определяет, что учетная запись пользователя деактивирована менеджером сайта
		 * @param iUmiObject $user объект пользователя
		 * @return bool
		 */
		private function isDeactivatedByManager(iUmiObject $user) : bool {
			return (bool) $user->getValue('is_deactivated_by_manager');
		}
		...
		private function getUserIdByEmail(string $email) : int {
			$selector = Service::SelectorFactory()
				->createObjectTypeName('users', 'user');
			$selector->where('e-mail')->equals($email);
			$selector->limit(0, 1);

			if (!$selector->first() instanceof iUmiObject) {
				throw new publicException(getLabel('label-no-users-found'));
			}

			return $selector->first()->getId();
		}
		 ...
		 * @throws selectorException
		 */
		private function getUserIdByLogin(string $login) : int {
			$selector = Service::SelectorFactory()
				->createObjectTypeName('users', 'user');
			$selector->where('login')->equals($login);
