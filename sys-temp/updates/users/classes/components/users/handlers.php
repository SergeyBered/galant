<?php
	...
	class UsersHandlers {
		...
		public function onLoginSuccess(iUmiEventPoint $event) {
			...
			$module = $this->module;
			$referrer = getRequest('from_page') ?: Service::Request()->referrer();
			$redirectToDefaultModule = Service::Registry()->get('//settings/default_module_admin_changed');

			if ($redirectToDefaultModule) {
				$moduleName = $module->getModuleForAdminEntrance();
			} else {
				preg_match('/\S*\/admin\/(\w*\/*\w*)\/*/', $referrer, $matches);
				$entryParts = explode('/', array_pop($matches));
				$moduleName = $entryParts[0];
				
				if (array_key_exists(1, $entryParts) && !empty($entryParts[1])) {
			...
			if (isset($methodName) && $permissions->isAllowedAdminEntryPoint($userId, $moduleName, $methodName)) {
				$module->redirect($referrer);
			}

			$methodName = Service::Registry()->get("//modules/{$moduleName}/default_method_admin");

			if ($methodName && $permissions->isAllowedAdminEntryPoint($userId, $moduleName, $methodName)) {
				$address = sprintf('%s/admin/%s/%s', $module->pre_lang, $moduleName, $methodName);
				$module->redirect($address);
			}
		...
		/**
		 * Изменяет значение поля "Деактивирован менеджером сайта" у объекта пользователя
		 * при активации/дезактивации пользователя через административную панель
		 * @param iUmiEventPoint $event событие изменения объекта пользователя
		 * @return void
		 * @throws coreException
		 */
		public function onSwitchUserActivity(iUmiEventPoint $event) : void {
			...
			$userId = $user->getId();
			$objectType = umiObjectTypesCollection::getInstance()->getType($user->getTypeId());

			if ($objectType->getModule() != 'users' || $objectType->getMethod() != 'user') {
				return;
			}

			$user->getValue('is_activated')
				? $user->setValue('is_deactivated_by_manager', 0)
				: $user->setValue('is_deactivated_by_manager', 1);
