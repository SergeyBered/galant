		...
		protected function getTemplateVariableList($template) {
			...
			foreach ($variableList as $variable) {
				...
				if (preg_match('/^((?!parse\.)[\w\-]+\.[^%]+)/', $variable)) {
					...
					$key = "mail-template-{$module}-variable-{$values[2]}";
					$label = getLabel($key, $module);
					$label = ($label === $key) ? getLabel("mail-template-variable-{$values[2]}", $module) : $label;
					$templateVariables[$variable] = $label;
				} else {
					$key = "mail-template-{$module}-variable-$variable";
					$label = getLabel($key, $module);
					$label = ($label === $key) ? getLabel("mail-template-variable-$variable", $module) : $label;
					$templateVariables[$variable] = $label;
				}
			}
