...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/result[@method = 'watermark']/data">
		<script type="text/javascript" src="/styles/skins/modern/data/modules/filemanager/watermark.js" />
	...
	<xsl:template name="watermark_preview">
		...
		<div id="preview_wrapper">
			...
			<div class="preview">
				...
				<img src="{$preview_src}?time={php:function('time')}" />
			</div>
		</div>
	</xsl:template>
	...
	<xsl:template match="option[@type = 'string' and starts-with(@name, 'image')]" mode="settings.modify-option">
		<div class="img_file watermark" id="{generate-id()}" umi:input-name="{@name}" umi:field-type="image_file"
			 umi:name="{@name}"
			 umi:lang="{/result/@interface-lang}"
			 umi:filemanager="elfinder"
			 umi:file="{value}"
			 umi:folder="./images/"
			 umi:file-hash="{value/@file_hash}"
			 umi:on_get_file_function="onChooseWaterMark">
			<xsl:choose>
				<xsl:when test="value/@folder_hash">
					<xsl:attribute name="umi:folder-hash"><xsl:value-of select="value/@folder_hash"/></xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="umi:folder-hash">umiimages</xsl:attribute>
				</xsl:otherwise>
			</xsl:choose>
			 

			<label for="imageField_{generate-id()}">
				<span class="layout-row-icon" id="imageField_{generate-id()}" />
			</label>
		</div>
	</xsl:template>
</xsl:stylesheet>
