<?php
	...
	class BlogsMacros {
		...
		public function blogsList($blogsCount = false, $sortType = false, $domainId = false, $template = 'default') {
			...
			$page = Service::Request()->pageNumber();
		...
		/**
		 * Возвращает теги поста блога
		 * @param int $postId идентификатор поста блога
		 * @param string $template имя шаблона (для tpl)
		 * @return mixed
		 * @throws publicException
		 */
		public function getPostTags($postId, $template = 'default') {
			$post = umiHierarchy::getInstance()
				->getElement($postId);
			...
			list($postTemplate) = blogs20::loadTemplates('blogs20/' . $template, 'posts_list_line');

			$result = [
				'subnodes:tags' => $this->prepareTags($post->getValue('tags'), $template)
			];

			return blogs20::parseTemplate($postTemplate, $result, $postId);
		}
		...
		public function prepareContent($content) {
			...
			$content = preg_replace("@\[smile:([0-9]+?)\]@i", '<img src="/styles/common/images/forum/smiles/$1.gif" alt="$1">', $content);
			return $content;
		}
