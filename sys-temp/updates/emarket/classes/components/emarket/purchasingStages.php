<?php
	 ...
	 *
	 * 5.3) Ожидание завершения оформления;
	...
	class EmarketPurchasingStages {
		...
		public $finishStageResults = ['fail', 'successful', 'wait'];
		...
		public function purchase($template = 'default') {
			...
			if (!$stage) {
				$order->order();
				$this->redirectToSuccessfulResult();
			}
			...
			if (!$step || !$byUrl) {
				$step = $step ?: 'choose';
				$this->redirectToPurchaseUrl([$stage, $step]);
			}
		...
		public function autofill(order $order, $service) {
			...
			$this->redirectToPurchaseUrl(['required']);
		}
		...
		/**
		 * Перенаправляет на url удачного завершения заказа
		 * @throws coreException
		 */
		public function redirectToSuccessfulResult() {
			$this->redirectToPurchaseUrl(['result', 'successful']);
		}

		/**
		 * Перенаправляет на url неудачного завершения заказа
		 * @throws coreException
		 */
		public function redirectToFailResult() {
			$this->redirectToPurchaseUrl(['result', 'fail']);
		}

		/**
		 * Перенаправляет на url оформления заказа в зависимости от переданных частей url
		 * @param string[] $pathParts части адреса url
		 * @throws coreException
		 */
		public function redirectToPurchaseUrl(array $pathParts) {
			$controller = cmsController::getInstance();
			$urlPrefix = $controller->getUrlPrefix() ? ($controller->getUrlPrefix() . '/') : '';

			$this->module->redirect(
				"{$this->module->pre_lang}/{$urlPrefix}emarket/purchase/" . implode('/', $pathParts) . '/'
			);
		}
	}
