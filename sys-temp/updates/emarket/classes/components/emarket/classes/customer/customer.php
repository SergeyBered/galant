<?php
	...
	class customer extends umiObjectProxy {
		...
		public function merge(iUmiObject $customer) {
			...
			$this->moveFieldsValuesToUser($customer);
		 ...
		 * @throws coreException
		 */
		public static function createGuestCustomer() : int {
		...
		/**
		 * Зачисляет заданное количество бонусов
		 * @param mixed $amount количество бонусов
		 * @return $this
		 * @throws privateException
		 */
		public function creditBonuses($amount) {
			$this->appendCreditedBonuses($amount);
			$this->commit();
			return $this;
		}

		/**
		 * Списывает заданное количество бонусов
		 * @param mixed $amount количество бонусов
		 * @return $this
		 * @throws privateException
		 */
		public function debitBonuses($amount) {
			$this->reduceCreditedBonuses($amount);
			$this->appendDebitedBonuses($amount);
			$this->commit();
			return $this;
		}

		/**
		 * Возвращает заданное количество бонусов
		 * @param mixed $amount количество бонусов
		 * @return $this
		 * @throws privateException
		 */
		public function returnBonuses($amount) {
			$this->appendCreditedBonuses($amount);
			$this->reduceDebitedBonuses($amount);
			$this->commit();
			return $this;
		}

		/**
		 * Увеличивает количество зачисленных бонусов на заданную сумму
		 * @param mixed $amount сумма увеличения
		 * @return $this
		 * @throws privateException
		 */
		private function appendCreditedBonuses($amount) {
			if ($amount < 0) {
				throw new privateException(getLabel('label-error-customer-append-bonus'));
			}

			$this->setCreditedBonuses($this->getCreditedBonuses() + $amount);
			return $this;
		}

		/**
		 * Уменьшает количество зачисленных бонусов на заданную сумму
		 * @param mixed $amount сумма уменьшения
		 * @return $this
		 * @throws privateException
		 */
		private function reduceCreditedBonuses($amount) {
			$existAmount = $this->getCreditedBonuses();

			if ($existAmount < $amount) {
				throw new privateException(getLabel('label-error-customer-reduce-bonus'));
			}

			$this->setCreditedBonuses($existAmount - $amount);
			return $this;
		}

		/**
		 * Устанавливает количество зачисленных бонусов
		 * @param mixed $amount количество
		 * @return bool
		 * @throws privateException
		 */
		private function setCreditedBonuses($amount) {
			if ($amount < 0) {
				throw new privateException(getLabel('label-error-customer-set-bonus'));
			}

			return $this->getObject()
				->setValue('bonus', $amount);
		}

		/**
		 * Возвращает количество зачисленных бонусов
		 * @return false|mixed
		 */
		private function getCreditedBonuses() {
			return $this->getObject()
				->getValue('bonus');
		}

		/**
		 * Увеличивает количество потраченных бонусов на заданную сумму
		 * @param mixed $amount сумма увеличения
		 * @return $this
		 * @throws privateException
		 */
		private function appendDebitedBonuses($amount) {
			if ($amount < 0) {
				throw new privateException(getLabel('label-error-customer-append-spent-bonus'));
			}

			$this->setDebitedBonuses($this->getDebitedBonuses() + $amount);
			return $this;
		}

		/**
		 * Уменьшает количество потраченных бонусов на заданную сумму
		 * @param mixed $amount сумма уменьшения
		 * @return $this
		 * @throws privateException
		 */
		private function reduceDebitedBonuses($amount) {
			$existAmount = $this->getDebitedBonuses();

			if ($existAmount < $amount) {
				throw new privateException(getLabel('label-error-customer-reduce-spent-bonus'));
			}

			$this->setDebitedBonuses($existAmount - $amount);
			return $this;
		}

		/**
		 * Устанавливает количество потраченных бонусов
		 * @param mixed $amount количество
		 * @return bool
		 * @throws privateException
		 */
		private function setDebitedBonuses($amount) {
			if ($amount < 0) {
				throw new privateException(getLabel('label-error-customer-set-spent-bonus'));
			}

			return $this->getObject()
				->setValue('spent_bonus', $amount);
		}

		/**
		 * Возвращает количество потраченных бонусов
		 * @return false|mixed
		 */
		private function getDebitedBonuses() {
			return $this->getObject()
				->getValue('spent_bonus');
		}
		...
		/**
		 * Переносит значения полей из незарегистрированного покупателя в текущего пользователя
		 * @param iUmiObject $customer объект незарегистрированного покупателя
		 * @return $this
		 */
		private function moveFieldsValuesToUser(iUmiObject $customer) {
			$fieldsNamesList = [
				self::LAST_NAME_FIELD,
				self::FIRST_NAME_FIELD,
				self::FATHER_NAME_FIELD,
				self::PHONE_NAME_FIELD
			];
	
			$user = self::getCustomerObject($this->getId());

			foreach ($fieldsNamesList as $fieldName) {
				$fieldValue = $customer->getValue($fieldName);
				if (!$user->getValue($fieldName) && $fieldValue) {
