<?php
	...
	class orderItem extends umiObjectProxy {
		 ...
		 * @param int|null $priceTypeId идентификатор типа цены
		 ...
		 */
		public static function createTradeOffer($elementId, $tradeOfferId = null, $priceTypeId = null) {
			$orderItem = self::createByClassPrefix($elementId, self::TRADE_OFFER_PREFIX);

			if ($tradeOfferId !== null) {
				$orderItem->setPriceTypeId($priceTypeId)
					->setOfferId($tradeOfferId);
				$orderItem->commit();
			}
		...
		private static function validateProductPage($id) {
			...
			if (Service::Registry()->get('//modules/emarket/put-any-type-pages-into-basket')) {
				return;
			}
		...
		protected static function setProperties($item, $element) {
			...
			if (!is_numeric($taxRateId) || $taxRateId == 0) {
				$taxRateId = (float) $emarket->getDefaultVatId();
			}
