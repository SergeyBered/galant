<?php
	...
	class TradeOfferOrderItem extends orderItem {
		...
		public function getElementPrice() {
			$price = $this->offer->getPriceCollection()->getMain();
			return $price ? $price->getValue() : (float) 0;
		}

		/**
		 * Устанавливает идентификатор типа цены торгового предложения
		 * @param int $priceTypeId идентификатор типа цены
		 * @return $this
		 */
		...
		protected function setOfferProperties(iOffer $offer) {
			...
			if (!is_numeric($weight) || $weight == 0) {
				$weight = (float) $settings->get(Settings::ORDER_ITEM_SECTION, 'weight');
			}
			$object->setValue('weight', $weight);

			$width = $offer->getWidth();
			if (!is_numeric($width) || $width == 0) {
				$width = (float) $settings->get(Settings::ORDER_ITEM_SECTION, 'width');
			}
			$object->setValue('width', $width);

			$height = $offer->getHeight();
			if (!is_numeric($height) || $height == 0) {
				$height = (float) $settings->get(Settings::ORDER_ITEM_SECTION, 'height');
			}
			$object->setValue('height', $height);

			$length = $offer->getLength();
			if (!is_numeric($length) || $length == 0) {
				$length = (float) $settings->get(Settings::ORDER_ITEM_SECTION, 'length');
			}
			$object->setValue('length', $length);

			$priceCollection = $offer->getPriceCollection();
			if ($this->price_type_id) {
				$price = $priceCollection->getByTypeId($this->price_type_id);
			} else {
				$price = $priceCollection->getMain();
			}

			if ($price instanceof iPrice) {
				$this->setOriginalPrice($price->getValue());
			}
