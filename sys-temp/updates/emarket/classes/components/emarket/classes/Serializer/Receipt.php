<?php
	...
	abstract class Receipt implements iReceipt {
		...
		protected function getOrderItemPrice(\order $order, \orderItem $orderItem) {
			...
			return round($orderItemPrice, 2, PHP_ROUND_HALF_DOWN);
		}
		 ...
		 * @return string
		 */
		protected function formatQuantity(int $quantity) : string {
			return sprintf('%.3f', $quantity);
		}

		/**
		 * Форматирует цену позиции заказа
		...
		protected function getDefaultOrderSetting($name) {
			...
			if ($settings instanceof Settings) {
				return $settings->get(Settings::ORDER_ITEM_SECTION, $name);
			}
