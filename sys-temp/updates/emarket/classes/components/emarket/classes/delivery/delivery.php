<?php
	...
	abstract class delivery extends umiObjectProxy {
		...
		/** @const string SELF_DELIVERY_DEFAULT_TYPE_ID идентификатор типа способа доставки "Самовывоз" по-умолчанию */
		const SELF_DELIVERY_DEFAULT_TYPE_ID = 'self';
		...
		final public static function getList($selfDeliveryOnly = false) {
			...
			if ($deliveryWithAddress) {
				...
				$selfDeliveryTypes = $config->get('modules', 'emarket.self-delivery.types');

				if (is_null($selfDeliveryTypes)) {
					$selfDeliveryTypes = self::SELF_DELIVERY_DEFAULT_TYPE_ID;
				}

				$types = [];
				$typesSel = new selector('objects');
				$typesSel->types('object-type')->guid('emarket-deliverytype');
				$typesSel->where('class_name')->equals($selfDeliveryTypes);
