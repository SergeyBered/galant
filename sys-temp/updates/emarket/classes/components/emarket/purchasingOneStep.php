<?php
	...
	class EmarketPurchasingOneStep {
		...
		public function purchasing_one_step($template = 'onestep') {
			...
			$module->getOrderValidator()
				->validateForPurchasing($order);
		 ...
		 * @param bool $isProcessOrder оформлять ли заказ
		 * @param bool $customerRequired является ли заполнение полей покупателя обязательным
		 * @return array|void
		 * @throws coreException
		 * @throws ErrorException
		...
		public function saveInfo(
			...
			$module = $this->module;
			$module->saveCustomer($customerRequired);
			...
			return [
				...
				'address-id' => $order->getDeliveryAddressId(),
				'delivery-id' => $order->getDeliveryId(),
		 ...
		 * @throws ErrorException
		 */
		public function saveCustomer(bool $customerRequired = true) : void {
			/** @var DataForms|FormSaver $data */
			$data = cmsController::getInstance()->getModule('data');
			$customer = customer::get();
			$data->saveEditedObject($customer->getId());

			if ($customerRequired && !$customer->isFilled() && !$customer->isUser()) {
				$this->module->errorNewMessage(getLabel('label-error-empty-customer', 'emarket'));
				$this->module->errorPanic();
			}
		}

		/**
		 * Возвращает цену текущего способа доставки
		 * @return mixed|int
		 * @throws coreException|umiRemoteFileGetterException
		 */
		public function getDeliveryPrice() {
			$order = $this->module->getBasketOrder(false);
			$delivery = delivery::get($order->getDeliveryId());

			return ($delivery instanceof delivery) ? $delivery->getDeliveryPrice($order) : 0;
		}

		/**
		 * Возвращает цену способа доставки по его идентификатору
		 * @param int $deliveryId идентификатор доставки
		 * @return mixed|int
		 * @throws coreException
		 * @throws umiRemoteFileGetterException
		 */
		public function getDeliveryPriceByDeliveryId(int $deliveryId = null) {
		 ...
		 * @param bool $addressRequired является ли выбор адреса обязательным
		 * @throws coreException|errorPanicException|privateException|ErrorException
		 */
		public function processOneStepAddress($order, $addressRequired) {
			...
			if ($deliveryId && delivery::get($deliveryId) instanceof selfDelivery) {
				return;
			}
		 ...
		 * @throws coreException|errorPanicException|privateException|publicException
		 */
		public function saveAddress(order $order, $addressId) {
			...
			if (Service::Request()->Post()->get('replace-address')) {
				$umiObjects->delObject($addressId);
				$addressId = 'new';
			}
			...
			if ($addressId == 'new') {
				...
				$data->saveEditedObjectWithIgnorePermissions($addressId, false, true);
			}
			...
			if (!$address->getObject()->isFilled()) {
				$address->delete();
				throw new publicException(getLabel('label-error-empty-address'));
			}
		...
		/**
		 * Сохраняет данные в заказ
		 * @param order $order заказ
		 */
		public function saveOneStepOrder(order $order) {
			$orderComment = Service::Request()->Post()->get('order_comment');
