<?php
	...
	class Client extends HttpClient implements iClient {
		...
		public function getLastRevision() {
			try {
				return $this->getLastReleaseInfo('revision');
		...
		public function getLastVersion() {
			try {
				return $this->getLastReleaseInfo('version');
			} catch (\Exception $exception) {
				\umiExceptionHandler::report($exception);
				return 0;
			}
		...
		public function getSupportEndTime() {
			...
			try {
				...
				\umiExceptionHandler::report($exception);
				return $this->getDateFactory()->create();
			}
		...
		/**
		 * Возвращает данные из информации о последнем релизе
		 * @param string $name имя параметра
		 * @return mixed
		 * @throws BadResponse
		 */
		private function getLastReleaseInfo($name) {
			$cacheEngine = $this->cacheEngine;
			$key = "last-release-$name";
			$value = $cacheEngine->loadRawData($key);

			if (is_numeric($value)) {
				return $value;
			}

			$releaseInfoKey = 'last-release-info';
			$response = $cacheEngine->loadRawData($releaseInfoKey);
			...
			$revisionList = $response->xpath("/modules/system/$name");
			$value = getFirstValue($revisionList);
			...
			$threeDayInSeconds = 3 * 24 * 3600;
			$cacheEngine->saveRawData($key, $value, $threeDayInSeconds);
			$cacheEngine->saveRawData($releaseInfoKey, $response->asXML(), $threeDayInSeconds);
