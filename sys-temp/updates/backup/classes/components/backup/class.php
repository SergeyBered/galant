<?php
	...
	class backup extends def_module {
		...
		public function createSnapshot() {
			...
			if (isDemoMode()) {
				$this->handleSnapshotError($buffer, getLabel('error-disabled-in-demo'), $location);
			}
			...
			try {
				...
				$this->handleSnapshotError($buffer, $exception->getMessage());
			}
		...
		/**
		 * Обрабатывает ошибку при работе с бекапом
		 * @param iOutputBuffer $buffer буффер вывода
		 * @param string $message сообщение об ошибке
		 * @param string|null $location адрес перенаправления
		 */
		private function handleSnapshotError(\iOutputBuffer $buffer, $message, $location = null) {
			$buffer->push("reportJsonError('{$message}');");

			if ($location) {
				$buffer->push("window.location = '{$location}';");
			}
