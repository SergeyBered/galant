...
var AdminIndexing = (function($){
	...
	Controller.onRenderComplete = function() {
		...
		if (indexButton.$elements) {
			if (indexButton.$elements.length) {
				$.each(indexButton.$elements, function(index, el) {
					$(el).on('click', Settings.buttons.indexIt.onClick);
				})
		...
		$namesLinks.on('click', function(e) {
			e.preventDefault();
		...
		function onLoadDeleteButton(response) {
			...
			$deleteButtons.on('click', Settings.buttons.delete.onClick);
		}
	};
	...
	$(function() {
		...
		$(Settings.buttons.addToIndex.selector).on('click', function(e) {
