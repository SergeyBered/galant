<?php
	...
	class CatalogHandlers {
		...
		public function updateIndexOnModifyPropertyByFastEdit(iUmiEventPoint $event) {
			...
			if (!$this->isNeedToReIndexWhenFieldEdited($field)) {
				return false;
			}
		...
		/**
		 * Обработчик изменения значения поля сущности через eip.
		 * Инициирует обновление индекса фильтров при изменения поля сущности
		...
		/**
		 * Определяет необходимость переиндексации фильтров при изменении определенного поля
		 * @param iUmiField $field поле
		 * @return bool
		 */
		private function isNeedToReIndexWhenFieldEdited(iUmiField $field) : bool {
			$isCommonQuantity = $field->getName() == 'common_quantity';
			$hideMissingProducts = (bool) Service::Registry()->get('//modules/catalog/disable_missing_products');

			if ($isCommonQuantity && $hideMissingProducts) {
				return true;
			}

			return $field->getIsInFilter();
		}
	}
