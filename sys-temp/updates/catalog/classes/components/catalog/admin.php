<?php
	...
	class CatalogAdmin implements iUmiRegistryInjector {
		...
		public function edit() {
			...
			$this->setTranslatorOption('field-name-black-list', ['trade_offer_list']);
			$data = $this->prepareData($inputData, 'page');
			$data = $this->append1cId($data, $element->getId());
			$this->setData($data);
			$this->doData();
		}
		...
		/**
		 * Возвращает дополненную 1С-идентификатором информацию об объекте каталога
		 * @param array $data
		 * @param int $pageId
		 * @return array
		 */
		private function append1cId(array $data, int $pageId) : array {
			/** @var exchange $exchange */
			$exchange = cmsController::getInstance()
				->getModule('exchange');
			...
			$editorAllowed =
				$permissions->isAllowedModule($userId, 'exchange') &&
				$permissions->isAllowedMethod($userId, 'exchange', 'identifiers');

			if (!$editorAllowed) {
				return $data;
			}

			$cmlSourceId = $exchange->getCmlSourceId();

			try {
				$identifier = umiImportRelations::getInstance()
					->getOldIdRelation($cmlSourceId, $pageId);
			} catch (databaseException $exception) {
				return $data;
			}

			$data['id-1c'] = $identifier ?: getLabel('label-no-1c-identifier');
			return $data;
		}
	}
