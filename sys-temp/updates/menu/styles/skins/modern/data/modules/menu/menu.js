...
jQuery(function(){
	...
	var listObj = JSON.parse(jQuery('#menuhierarchy').val() || 'null');

	if(typeof listObj =='object' && listObj !== null) menuDecode(listObj);

    jQuery('.dd').data('output', jQuery('#menuhierarchy'));	
	// сортировка меню
	jQuery('.dd').nestable().on('change',updateOutput);
	
	// удаление пункта меню
	jQuery('.menuItemDel').off();
	...
	jQuery('#submit_customlink').on('click', function(){
		var name='';
		var link='';
		
		name=jQuery('#custom-name').val();
		link=jQuery('#custom-url').val().trim();
	...
	jQuery('.systemLink').on('click', function(){
		var name='';
		var link='';
		
		name=jQuery(this).text().trim();
		link=jQuery(this).attr('href').trim();
