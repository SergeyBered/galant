<?php
	...
	class DataAdmin {
		...
		public function types() {
			...
			$currentPageNumber = Service::Request()->pageNumber();
		...
		public function type_field_edit() {
			...
			if ($this->isSaveMode('param2')) {
				$this->saveEditedFieldData($field, $typeId);
				if (!getRequest('noredirect')) {
					$this->chooseRedirect();
				}
			}
		...
		public function getSameFieldFromRelatedTypes($typeId = null, array $fieldData = []) {
			...
			if (empty($fieldIdAndTypeId)) {
				...
				if (in_array($typeId, $siblingTypeIdList)) {
					$index = array_search($typeId, $siblingTypeIdList);
					unset($siblingTypeIdList[$index]);
				}
		...
		public function guide_items($guideId = false, $perPage = false, $currentPage = 0) {
			$currentPage = $currentPage ?: Service::Request()->pageNumber();
		...
		public function guide_items_all() {
			...
			$pageNavigationNumber = Service::Request()->pageNumber();
		...
		/**
		 * Выводит в буфер список языков в формате содержимого справочника.
		 * Часть api для табличного контрола.
		 */
		public function getLangsList() : void {
			if ($this->module->ifNotXmlMode()) {
				$this->setDirectCallError();
				$this->doData();
				return;
			...
			$langList = Service::LanguageCollection()
				->getList();
			$langNodeList = [];
			...
			foreach ($langList as $lang) {
				$langNodeList[] = [
					...
					'attribute:name' => $lang->getTitle()
				];
			}

			$result = [
				'nodes:object' => $langNodeList
			];

			$this->printEntityTableControlResult($result, count($langList));
		}
...
SQL;
		...
		/**
		 * Возвращает список элементов справочника 
		 * для полей "Способ оплаты" и "Способ доставки" с учетом домена
		 * @throws coreException
		 * @throws selectorException
		 */
		public function guideItemsForDomain() {
			$this->setDataType('list');
			$this->setActionType('modify');
			$module = $this->module;
			...
			$guideId = (int) getRequest('param0');
			$domainId = (int) getRequest('domainId');
			$selector = Service::SelectorFactory()
				->createObjectTypeId($guideId);
			$selector = $module->appendGuideListDomainFilter($selector, $domainId);
			$selector->option('return')->value('id');
		...
		/**
		 * Добавляет в выборку количества элементов справочника фильтрацию по списку доменов
		 * @param selector $selector выборка
		 * @param int $domainId идетификатор домена
		 * @return selector
		 * @throws selectorException
		 */
		public function appendGuideListDomainFilter(selector $selector, int $domainId) : selector {
			if (!$domainId) return $selector;

			$selector->option('or-mode')->field('domain_id_list');
